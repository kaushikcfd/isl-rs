// Automatically generated by isl_bindings_generator.
// LICENSE: MIT

use super::{
    Aff, AffList, BasicSet, Context, DimType, Error, Id, LibISLError, Map, MultiId, MultiPwAff,
    MultiUnionPwAff, MultiVal, PwMultiAff, Set, Space, Val,
};
use libc::uintptr_t;
use std::ffi::{CStr, CString};
use std::os::raw::c_char;

/// Wraps `isl_multi_aff`.
pub struct MultiAff {
    pub ptr: uintptr_t,
    pub should_free_on_drop: bool,
}

extern "C" {

    fn isl_multi_aff_add(multi1: uintptr_t, multi2: uintptr_t) -> uintptr_t;

    fn isl_multi_aff_add_constant_multi_val(mpa: uintptr_t, mv: uintptr_t) -> uintptr_t;

    fn isl_multi_aff_add_constant_val(mpa: uintptr_t, v: uintptr_t) -> uintptr_t;

    fn isl_multi_aff_add_dims(multi: uintptr_t, type_: i32, n: u32) -> uintptr_t;

    fn isl_multi_aff_align_params(multi: uintptr_t, model: uintptr_t) -> uintptr_t;

    fn isl_multi_aff_as_map(ma: uintptr_t) -> uintptr_t;

    fn isl_multi_aff_as_set(ma: uintptr_t) -> uintptr_t;

    fn isl_multi_aff_bind(ma: uintptr_t, tuple: uintptr_t) -> uintptr_t;

    fn isl_multi_aff_bind_domain(multi: uintptr_t, tuple: uintptr_t) -> uintptr_t;

    fn isl_multi_aff_bind_domain_wrapped_domain(multi: uintptr_t, tuple: uintptr_t) -> uintptr_t;

    fn isl_multi_aff_copy(multi: uintptr_t) -> uintptr_t;

    fn isl_multi_aff_dim(multi: uintptr_t, type_: i32) -> i32;

    fn isl_multi_aff_domain_map(space: uintptr_t) -> uintptr_t;

    fn isl_multi_aff_domain_reverse(multi: uintptr_t) -> uintptr_t;

    fn isl_multi_aff_drop_dims(multi: uintptr_t, type_: i32, first: u32, n: u32) -> uintptr_t;

    fn isl_multi_aff_dump(maff: uintptr_t) -> ();

    fn isl_multi_aff_factor_range(multi: uintptr_t) -> uintptr_t;

    fn isl_multi_aff_find_dim_by_id(multi: uintptr_t, type_: i32, id: uintptr_t) -> i32;

    fn isl_multi_aff_find_dim_by_name(multi: uintptr_t, type_: i32, name: *const c_char) -> i32;

    fn isl_multi_aff_flat_range_product(multi1: uintptr_t, multi2: uintptr_t) -> uintptr_t;

    fn isl_multi_aff_flatten_domain(ma: uintptr_t) -> uintptr_t;

    fn isl_multi_aff_flatten_range(multi: uintptr_t) -> uintptr_t;

    fn isl_multi_aff_floor(ma: uintptr_t) -> uintptr_t;

    fn isl_multi_aff_free(multi: uintptr_t) -> uintptr_t;

    fn isl_multi_aff_from_aff(aff: uintptr_t) -> uintptr_t;

    fn isl_multi_aff_from_aff_list(space: uintptr_t, list: uintptr_t) -> uintptr_t;

    fn isl_multi_aff_from_range(multi: uintptr_t) -> uintptr_t;

    fn isl_multi_aff_get_aff(multi: uintptr_t, pos: i32) -> uintptr_t;

    fn isl_multi_aff_get_at(multi: uintptr_t, pos: i32) -> uintptr_t;

    fn isl_multi_aff_get_constant_multi_val(ma: uintptr_t) -> uintptr_t;

    fn isl_multi_aff_get_ctx(multi: uintptr_t) -> uintptr_t;

    fn isl_multi_aff_get_dim_id(multi: uintptr_t, type_: i32, pos: u32) -> uintptr_t;

    fn isl_multi_aff_get_domain_space(multi: uintptr_t) -> uintptr_t;

    fn isl_multi_aff_get_list(multi: uintptr_t) -> uintptr_t;

    fn isl_multi_aff_get_range_tuple_id(multi: uintptr_t) -> uintptr_t;

    fn isl_multi_aff_get_space(multi: uintptr_t) -> uintptr_t;

    fn isl_multi_aff_get_tuple_id(multi: uintptr_t, type_: i32) -> uintptr_t;

    fn isl_multi_aff_get_tuple_name(multi: uintptr_t, type_: i32) -> *const c_char;

    fn isl_multi_aff_gist(maff: uintptr_t, context: uintptr_t) -> uintptr_t;

    fn isl_multi_aff_gist_params(maff: uintptr_t, context: uintptr_t) -> uintptr_t;

    fn isl_multi_aff_has_range_tuple_id(multi: uintptr_t) -> i32;

    fn isl_multi_aff_has_tuple_id(multi: uintptr_t, type_: i32) -> i32;

    fn isl_multi_aff_identity(space: uintptr_t) -> uintptr_t;

    fn isl_multi_aff_identity_multi_aff(multi: uintptr_t) -> uintptr_t;

    fn isl_multi_aff_identity_on_domain_space(space: uintptr_t) -> uintptr_t;

    fn isl_multi_aff_insert_dims(multi: uintptr_t, type_: i32, first: u32, n: u32) -> uintptr_t;

    fn isl_multi_aff_insert_domain(multi: uintptr_t, domain: uintptr_t) -> uintptr_t;

    fn isl_multi_aff_involves_dims(multi: uintptr_t, type_: i32, first: u32, n: u32) -> i32;

    fn isl_multi_aff_involves_locals(multi: uintptr_t) -> i32;

    fn isl_multi_aff_involves_nan(multi: uintptr_t) -> i32;

    fn isl_multi_aff_lex_ge_set(ma1: uintptr_t, ma2: uintptr_t) -> uintptr_t;

    fn isl_multi_aff_lex_gt_set(ma1: uintptr_t, ma2: uintptr_t) -> uintptr_t;

    fn isl_multi_aff_lex_le_set(ma1: uintptr_t, ma2: uintptr_t) -> uintptr_t;

    fn isl_multi_aff_lex_lt_set(ma1: uintptr_t, ma2: uintptr_t) -> uintptr_t;

    fn isl_multi_aff_mod_multi_val(multi: uintptr_t, mv: uintptr_t) -> uintptr_t;

    fn isl_multi_aff_move_dims(ma: uintptr_t, dst_type: i32, dst_pos: u32, src_type: i32,
                               src_pos: u32, n: u32)
                               -> uintptr_t;

    fn isl_multi_aff_multi_val_on_domain_space(space: uintptr_t, mv: uintptr_t) -> uintptr_t;

    fn isl_multi_aff_multi_val_on_space(space: uintptr_t, mv: uintptr_t) -> uintptr_t;

    fn isl_multi_aff_neg(multi: uintptr_t) -> uintptr_t;

    fn isl_multi_aff_plain_cmp(multi1: uintptr_t, multi2: uintptr_t) -> i32;

    fn isl_multi_aff_plain_is_equal(multi1: uintptr_t, multi2: uintptr_t) -> i32;

    fn isl_multi_aff_product(multi1: uintptr_t, multi2: uintptr_t) -> uintptr_t;

    fn isl_multi_aff_project_domain_on_params(multi: uintptr_t) -> uintptr_t;

    fn isl_multi_aff_project_out_map(space: uintptr_t, type_: i32, first: u32, n: u32)
                                     -> uintptr_t;

    fn isl_multi_aff_pullback_multi_aff(ma1: uintptr_t, ma2: uintptr_t) -> uintptr_t;

    fn isl_multi_aff_range_factor_domain(multi: uintptr_t) -> uintptr_t;

    fn isl_multi_aff_range_factor_range(multi: uintptr_t) -> uintptr_t;

    fn isl_multi_aff_range_is_wrapping(multi: uintptr_t) -> i32;

    fn isl_multi_aff_range_map(space: uintptr_t) -> uintptr_t;

    fn isl_multi_aff_range_product(multi1: uintptr_t, multi2: uintptr_t) -> uintptr_t;

    fn isl_multi_aff_range_splice(multi1: uintptr_t, pos: u32, multi2: uintptr_t) -> uintptr_t;

    fn isl_multi_aff_read_from_str(ctx: uintptr_t, str_: *const c_char) -> uintptr_t;

    fn isl_multi_aff_reset_range_tuple_id(multi: uintptr_t) -> uintptr_t;

    fn isl_multi_aff_reset_tuple_id(multi: uintptr_t, type_: i32) -> uintptr_t;

    fn isl_multi_aff_reset_user(multi: uintptr_t) -> uintptr_t;

    fn isl_multi_aff_scale_down_multi_val(multi: uintptr_t, mv: uintptr_t) -> uintptr_t;

    fn isl_multi_aff_scale_down_val(multi: uintptr_t, v: uintptr_t) -> uintptr_t;

    fn isl_multi_aff_scale_multi_val(multi: uintptr_t, mv: uintptr_t) -> uintptr_t;

    fn isl_multi_aff_scale_val(multi: uintptr_t, v: uintptr_t) -> uintptr_t;

    fn isl_multi_aff_set_aff(multi: uintptr_t, pos: i32, el: uintptr_t) -> uintptr_t;

    fn isl_multi_aff_set_at(multi: uintptr_t, pos: i32, el: uintptr_t) -> uintptr_t;

    fn isl_multi_aff_set_dim_id(multi: uintptr_t, type_: i32, pos: u32, id: uintptr_t)
                                -> uintptr_t;

    fn isl_multi_aff_set_dim_name(multi: uintptr_t, type_: i32, pos: u32, s: *const c_char)
                                  -> uintptr_t;

    fn isl_multi_aff_set_range_tuple_id(multi: uintptr_t, id: uintptr_t) -> uintptr_t;

    fn isl_multi_aff_set_tuple_id(multi: uintptr_t, type_: i32, id: uintptr_t) -> uintptr_t;

    fn isl_multi_aff_set_tuple_name(multi: uintptr_t, type_: i32, s: *const c_char) -> uintptr_t;

    fn isl_multi_aff_size(multi: uintptr_t) -> i32;

    fn isl_multi_aff_splice(multi1: uintptr_t, in_pos: u32, out_pos: u32, multi2: uintptr_t)
                            -> uintptr_t;

    fn isl_multi_aff_sub(multi1: uintptr_t, multi2: uintptr_t) -> uintptr_t;

    fn isl_multi_aff_to_multi_pw_aff(ma: uintptr_t) -> uintptr_t;

    fn isl_multi_aff_to_multi_union_pw_aff(ma: uintptr_t) -> uintptr_t;

    fn isl_multi_aff_to_pw_multi_aff(ma: uintptr_t) -> uintptr_t;

    fn isl_multi_aff_to_str(ma: uintptr_t) -> *const c_char;

    fn isl_multi_aff_unbind_params_insert_domain(multi: uintptr_t, domain: uintptr_t) -> uintptr_t;

    fn isl_multi_aff_zero(space: uintptr_t) -> uintptr_t;

}

impl MultiAff {
    /// Wraps `isl_multi_aff_add`.
    pub fn add(self, multi2: MultiAff) -> Result<MultiAff, LibISLError> {
        let multi1 = self;
        let isl_rs_ctx = multi1.get_ctx();
        let mut multi1 = multi1;
        multi1.do_not_free_on_drop();
        let multi1 = multi1.ptr;
        let mut multi2 = multi2;
        multi2.do_not_free_on_drop();
        let multi2 = multi2.ptr;
        let isl_rs_result = unsafe { isl_multi_aff_add(multi1, multi2) };
        let isl_rs_result = MultiAff { ptr: isl_rs_result,
                                       should_free_on_drop: true };
        let err = isl_rs_ctx.last_error();
        if err != Error::None_ {
            return Err(LibISLError::new(err, isl_rs_ctx.last_error_msg()));
        }
        Ok(isl_rs_result)
    }

    /// Wraps `isl_multi_aff_add_constant_multi_val`.
    pub fn add_constant_multi_val(self, mv: MultiVal) -> Result<MultiAff, LibISLError> {
        let mpa = self;
        let isl_rs_ctx = mpa.get_ctx();
        let mut mpa = mpa;
        mpa.do_not_free_on_drop();
        let mpa = mpa.ptr;
        let mut mv = mv;
        mv.do_not_free_on_drop();
        let mv = mv.ptr;
        let isl_rs_result = unsafe { isl_multi_aff_add_constant_multi_val(mpa, mv) };
        let isl_rs_result = MultiAff { ptr: isl_rs_result,
                                       should_free_on_drop: true };
        let err = isl_rs_ctx.last_error();
        if err != Error::None_ {
            return Err(LibISLError::new(err, isl_rs_ctx.last_error_msg()));
        }
        Ok(isl_rs_result)
    }

    /// Wraps `isl_multi_aff_add_constant_val`.
    pub fn add_constant_val(self, v: Val) -> Result<MultiAff, LibISLError> {
        let mpa = self;
        let isl_rs_ctx = mpa.get_ctx();
        let mut mpa = mpa;
        mpa.do_not_free_on_drop();
        let mpa = mpa.ptr;
        let mut v = v;
        v.do_not_free_on_drop();
        let v = v.ptr;
        let isl_rs_result = unsafe { isl_multi_aff_add_constant_val(mpa, v) };
        let isl_rs_result = MultiAff { ptr: isl_rs_result,
                                       should_free_on_drop: true };
        let err = isl_rs_ctx.last_error();
        if err != Error::None_ {
            return Err(LibISLError::new(err, isl_rs_ctx.last_error_msg()));
        }
        Ok(isl_rs_result)
    }

    /// Wraps `isl_multi_aff_add_dims`.
    pub fn add_dims(self, type_: DimType, n: u32) -> Result<MultiAff, LibISLError> {
        let multi = self;
        let isl_rs_ctx = multi.get_ctx();
        let mut multi = multi;
        multi.do_not_free_on_drop();
        let multi = multi.ptr;
        let type_ = type_.to_i32();
        let isl_rs_result = unsafe { isl_multi_aff_add_dims(multi, type_, n) };
        let isl_rs_result = MultiAff { ptr: isl_rs_result,
                                       should_free_on_drop: true };
        let err = isl_rs_ctx.last_error();
        if err != Error::None_ {
            return Err(LibISLError::new(err, isl_rs_ctx.last_error_msg()));
        }
        Ok(isl_rs_result)
    }

    /// Wraps `isl_multi_aff_align_params`.
    pub fn align_params(self, model: Space) -> Result<MultiAff, LibISLError> {
        let multi = self;
        let isl_rs_ctx = multi.get_ctx();
        let mut multi = multi;
        multi.do_not_free_on_drop();
        let multi = multi.ptr;
        let mut model = model;
        model.do_not_free_on_drop();
        let model = model.ptr;
        let isl_rs_result = unsafe { isl_multi_aff_align_params(multi, model) };
        let isl_rs_result = MultiAff { ptr: isl_rs_result,
                                       should_free_on_drop: true };
        let err = isl_rs_ctx.last_error();
        if err != Error::None_ {
            return Err(LibISLError::new(err, isl_rs_ctx.last_error_msg()));
        }
        Ok(isl_rs_result)
    }

    /// Wraps `isl_multi_aff_as_map`.
    pub fn as_map(self) -> Result<Map, LibISLError> {
        let ma = self;
        let isl_rs_ctx = ma.get_ctx();
        let mut ma = ma;
        ma.do_not_free_on_drop();
        let ma = ma.ptr;
        let isl_rs_result = unsafe { isl_multi_aff_as_map(ma) };
        let isl_rs_result = Map { ptr: isl_rs_result,
                                  should_free_on_drop: true };
        let err = isl_rs_ctx.last_error();
        if err != Error::None_ {
            return Err(LibISLError::new(err, isl_rs_ctx.last_error_msg()));
        }
        Ok(isl_rs_result)
    }

    /// Wraps `isl_multi_aff_as_set`.
    pub fn as_set(self) -> Result<Set, LibISLError> {
        let ma = self;
        let isl_rs_ctx = ma.get_ctx();
        let mut ma = ma;
        ma.do_not_free_on_drop();
        let ma = ma.ptr;
        let isl_rs_result = unsafe { isl_multi_aff_as_set(ma) };
        let isl_rs_result = Set { ptr: isl_rs_result,
                                  should_free_on_drop: true };
        let err = isl_rs_ctx.last_error();
        if err != Error::None_ {
            return Err(LibISLError::new(err, isl_rs_ctx.last_error_msg()));
        }
        Ok(isl_rs_result)
    }

    /// Wraps `isl_multi_aff_bind`.
    pub fn bind(self, tuple: MultiId) -> Result<BasicSet, LibISLError> {
        let ma = self;
        let isl_rs_ctx = ma.get_ctx();
        let mut ma = ma;
        ma.do_not_free_on_drop();
        let ma = ma.ptr;
        let mut tuple = tuple;
        tuple.do_not_free_on_drop();
        let tuple = tuple.ptr;
        let isl_rs_result = unsafe { isl_multi_aff_bind(ma, tuple) };
        let isl_rs_result = BasicSet { ptr: isl_rs_result,
                                       should_free_on_drop: true };
        let err = isl_rs_ctx.last_error();
        if err != Error::None_ {
            return Err(LibISLError::new(err, isl_rs_ctx.last_error_msg()));
        }
        Ok(isl_rs_result)
    }

    /// Wraps `isl_multi_aff_bind_domain`.
    pub fn bind_domain(self, tuple: MultiId) -> Result<MultiAff, LibISLError> {
        let multi = self;
        let isl_rs_ctx = multi.get_ctx();
        let mut multi = multi;
        multi.do_not_free_on_drop();
        let multi = multi.ptr;
        let mut tuple = tuple;
        tuple.do_not_free_on_drop();
        let tuple = tuple.ptr;
        let isl_rs_result = unsafe { isl_multi_aff_bind_domain(multi, tuple) };
        let isl_rs_result = MultiAff { ptr: isl_rs_result,
                                       should_free_on_drop: true };
        let err = isl_rs_ctx.last_error();
        if err != Error::None_ {
            return Err(LibISLError::new(err, isl_rs_ctx.last_error_msg()));
        }
        Ok(isl_rs_result)
    }

    /// Wraps `isl_multi_aff_bind_domain_wrapped_domain`.
    pub fn bind_domain_wrapped_domain(self, tuple: MultiId) -> Result<MultiAff, LibISLError> {
        let multi = self;
        let isl_rs_ctx = multi.get_ctx();
        let mut multi = multi;
        multi.do_not_free_on_drop();
        let multi = multi.ptr;
        let mut tuple = tuple;
        tuple.do_not_free_on_drop();
        let tuple = tuple.ptr;
        let isl_rs_result = unsafe { isl_multi_aff_bind_domain_wrapped_domain(multi, tuple) };
        let isl_rs_result = MultiAff { ptr: isl_rs_result,
                                       should_free_on_drop: true };
        let err = isl_rs_ctx.last_error();
        if err != Error::None_ {
            return Err(LibISLError::new(err, isl_rs_ctx.last_error_msg()));
        }
        Ok(isl_rs_result)
    }

    /// Wraps `isl_multi_aff_copy`.
    pub fn copy(&self) -> Result<MultiAff, LibISLError> {
        let multi = self;
        let isl_rs_ctx = multi.get_ctx();
        let multi = multi.ptr;
        let isl_rs_result = unsafe { isl_multi_aff_copy(multi) };
        let isl_rs_result = MultiAff { ptr: isl_rs_result,
                                       should_free_on_drop: true };
        let err = isl_rs_ctx.last_error();
        if err != Error::None_ {
            return Err(LibISLError::new(err, isl_rs_ctx.last_error_msg()));
        }
        Ok(isl_rs_result)
    }

    /// Wraps `isl_multi_aff_dim`.
    pub fn dim(&self, type_: DimType) -> Result<i32, LibISLError> {
        let multi = self;
        let isl_rs_ctx = multi.get_ctx();
        let multi = multi.ptr;
        let type_ = type_.to_i32();
        let isl_rs_result = unsafe { isl_multi_aff_dim(multi, type_) };
        let err = isl_rs_ctx.last_error();
        if err != Error::None_ {
            return Err(LibISLError::new(err, isl_rs_ctx.last_error_msg()));
        }
        Ok(isl_rs_result)
    }

    /// Wraps `isl_multi_aff_domain_map`.
    pub fn domain_map(space: Space) -> Result<MultiAff, LibISLError> {
        let isl_rs_ctx = space.get_ctx();
        let mut space = space;
        space.do_not_free_on_drop();
        let space = space.ptr;
        let isl_rs_result = unsafe { isl_multi_aff_domain_map(space) };
        let isl_rs_result = MultiAff { ptr: isl_rs_result,
                                       should_free_on_drop: true };
        let err = isl_rs_ctx.last_error();
        if err != Error::None_ {
            return Err(LibISLError::new(err, isl_rs_ctx.last_error_msg()));
        }
        Ok(isl_rs_result)
    }

    /// Wraps `isl_multi_aff_domain_reverse`.
    pub fn domain_reverse(self) -> Result<MultiAff, LibISLError> {
        let multi = self;
        let isl_rs_ctx = multi.get_ctx();
        let mut multi = multi;
        multi.do_not_free_on_drop();
        let multi = multi.ptr;
        let isl_rs_result = unsafe { isl_multi_aff_domain_reverse(multi) };
        let isl_rs_result = MultiAff { ptr: isl_rs_result,
                                       should_free_on_drop: true };
        let err = isl_rs_ctx.last_error();
        if err != Error::None_ {
            return Err(LibISLError::new(err, isl_rs_ctx.last_error_msg()));
        }
        Ok(isl_rs_result)
    }

    /// Wraps `isl_multi_aff_drop_dims`.
    pub fn drop_dims(self, type_: DimType, first: u32, n: u32) -> Result<MultiAff, LibISLError> {
        let multi = self;
        let isl_rs_ctx = multi.get_ctx();
        let mut multi = multi;
        multi.do_not_free_on_drop();
        let multi = multi.ptr;
        let type_ = type_.to_i32();
        let isl_rs_result = unsafe { isl_multi_aff_drop_dims(multi, type_, first, n) };
        let isl_rs_result = MultiAff { ptr: isl_rs_result,
                                       should_free_on_drop: true };
        let err = isl_rs_ctx.last_error();
        if err != Error::None_ {
            return Err(LibISLError::new(err, isl_rs_ctx.last_error_msg()));
        }
        Ok(isl_rs_result)
    }

    /// Wraps `isl_multi_aff_dump`.
    pub fn dump(&self) -> Result<(), LibISLError> {
        let maff = self;
        let isl_rs_ctx = maff.get_ctx();
        let maff = maff.ptr;
        let isl_rs_result = unsafe { isl_multi_aff_dump(maff) };
        let err = isl_rs_ctx.last_error();
        if err != Error::None_ {
            return Err(LibISLError::new(err, isl_rs_ctx.last_error_msg()));
        }
        Ok(isl_rs_result)
    }

    /// Wraps `isl_multi_aff_factor_range`.
    pub fn factor_range(self) -> Result<MultiAff, LibISLError> {
        let multi = self;
        let isl_rs_ctx = multi.get_ctx();
        let mut multi = multi;
        multi.do_not_free_on_drop();
        let multi = multi.ptr;
        let isl_rs_result = unsafe { isl_multi_aff_factor_range(multi) };
        let isl_rs_result = MultiAff { ptr: isl_rs_result,
                                       should_free_on_drop: true };
        let err = isl_rs_ctx.last_error();
        if err != Error::None_ {
            return Err(LibISLError::new(err, isl_rs_ctx.last_error_msg()));
        }
        Ok(isl_rs_result)
    }

    /// Wraps `isl_multi_aff_find_dim_by_id`.
    pub fn find_dim_by_id(&self, type_: DimType, id: &Id) -> Result<i32, LibISLError> {
        let multi = self;
        let isl_rs_ctx = multi.get_ctx();
        let multi = multi.ptr;
        let type_ = type_.to_i32();
        let id = id.ptr;
        let isl_rs_result = unsafe { isl_multi_aff_find_dim_by_id(multi, type_, id) };
        let err = isl_rs_ctx.last_error();
        if err != Error::None_ {
            return Err(LibISLError::new(err, isl_rs_ctx.last_error_msg()));
        }
        Ok(isl_rs_result)
    }

    /// Wraps `isl_multi_aff_find_dim_by_name`.
    pub fn find_dim_by_name(&self, type_: DimType, name: &str) -> Result<i32, LibISLError> {
        let multi = self;
        let isl_rs_ctx = multi.get_ctx();
        let multi = multi.ptr;
        let type_ = type_.to_i32();
        let name = CString::new(name).unwrap();
        let name = name.as_ptr();
        let isl_rs_result = unsafe { isl_multi_aff_find_dim_by_name(multi, type_, name) };
        let err = isl_rs_ctx.last_error();
        if err != Error::None_ {
            return Err(LibISLError::new(err, isl_rs_ctx.last_error_msg()));
        }
        Ok(isl_rs_result)
    }

    /// Wraps `isl_multi_aff_flat_range_product`.
    pub fn flat_range_product(self, multi2: MultiAff) -> Result<MultiAff, LibISLError> {
        let multi1 = self;
        let isl_rs_ctx = multi1.get_ctx();
        let mut multi1 = multi1;
        multi1.do_not_free_on_drop();
        let multi1 = multi1.ptr;
        let mut multi2 = multi2;
        multi2.do_not_free_on_drop();
        let multi2 = multi2.ptr;
        let isl_rs_result = unsafe { isl_multi_aff_flat_range_product(multi1, multi2) };
        let isl_rs_result = MultiAff { ptr: isl_rs_result,
                                       should_free_on_drop: true };
        let err = isl_rs_ctx.last_error();
        if err != Error::None_ {
            return Err(LibISLError::new(err, isl_rs_ctx.last_error_msg()));
        }
        Ok(isl_rs_result)
    }

    /// Wraps `isl_multi_aff_flatten_domain`.
    pub fn flatten_domain(self) -> Result<MultiAff, LibISLError> {
        let ma = self;
        let isl_rs_ctx = ma.get_ctx();
        let mut ma = ma;
        ma.do_not_free_on_drop();
        let ma = ma.ptr;
        let isl_rs_result = unsafe { isl_multi_aff_flatten_domain(ma) };
        let isl_rs_result = MultiAff { ptr: isl_rs_result,
                                       should_free_on_drop: true };
        let err = isl_rs_ctx.last_error();
        if err != Error::None_ {
            return Err(LibISLError::new(err, isl_rs_ctx.last_error_msg()));
        }
        Ok(isl_rs_result)
    }

    /// Wraps `isl_multi_aff_flatten_range`.
    pub fn flatten_range(self) -> Result<MultiAff, LibISLError> {
        let multi = self;
        let isl_rs_ctx = multi.get_ctx();
        let mut multi = multi;
        multi.do_not_free_on_drop();
        let multi = multi.ptr;
        let isl_rs_result = unsafe { isl_multi_aff_flatten_range(multi) };
        let isl_rs_result = MultiAff { ptr: isl_rs_result,
                                       should_free_on_drop: true };
        let err = isl_rs_ctx.last_error();
        if err != Error::None_ {
            return Err(LibISLError::new(err, isl_rs_ctx.last_error_msg()));
        }
        Ok(isl_rs_result)
    }

    /// Wraps `isl_multi_aff_floor`.
    pub fn floor(self) -> Result<MultiAff, LibISLError> {
        let ma = self;
        let isl_rs_ctx = ma.get_ctx();
        let mut ma = ma;
        ma.do_not_free_on_drop();
        let ma = ma.ptr;
        let isl_rs_result = unsafe { isl_multi_aff_floor(ma) };
        let isl_rs_result = MultiAff { ptr: isl_rs_result,
                                       should_free_on_drop: true };
        let err = isl_rs_ctx.last_error();
        if err != Error::None_ {
            return Err(LibISLError::new(err, isl_rs_ctx.last_error_msg()));
        }
        Ok(isl_rs_result)
    }

    /// Wraps `isl_multi_aff_free`.
    pub fn free(self) -> Result<MultiAff, LibISLError> {
        let multi = self;
        let isl_rs_ctx = multi.get_ctx();
        let mut multi = multi;
        multi.do_not_free_on_drop();
        let multi = multi.ptr;
        let isl_rs_result = unsafe { isl_multi_aff_free(multi) };
        let isl_rs_result = MultiAff { ptr: isl_rs_result,
                                       should_free_on_drop: true };
        let err = isl_rs_ctx.last_error();
        if err != Error::None_ {
            return Err(LibISLError::new(err, isl_rs_ctx.last_error_msg()));
        }
        Ok(isl_rs_result)
    }

    /// Wraps `isl_multi_aff_from_aff`.
    pub fn from_aff(aff: Aff) -> Result<MultiAff, LibISLError> {
        let isl_rs_ctx = aff.get_ctx();
        let mut aff = aff;
        aff.do_not_free_on_drop();
        let aff = aff.ptr;
        let isl_rs_result = unsafe { isl_multi_aff_from_aff(aff) };
        let isl_rs_result = MultiAff { ptr: isl_rs_result,
                                       should_free_on_drop: true };
        let err = isl_rs_ctx.last_error();
        if err != Error::None_ {
            return Err(LibISLError::new(err, isl_rs_ctx.last_error_msg()));
        }
        Ok(isl_rs_result)
    }

    /// Wraps `isl_multi_aff_from_aff_list`.
    pub fn from_aff_list(space: Space, list: AffList) -> Result<MultiAff, LibISLError> {
        let isl_rs_ctx = space.get_ctx();
        let mut space = space;
        space.do_not_free_on_drop();
        let space = space.ptr;
        let mut list = list;
        list.do_not_free_on_drop();
        let list = list.ptr;
        let isl_rs_result = unsafe { isl_multi_aff_from_aff_list(space, list) };
        let isl_rs_result = MultiAff { ptr: isl_rs_result,
                                       should_free_on_drop: true };
        let err = isl_rs_ctx.last_error();
        if err != Error::None_ {
            return Err(LibISLError::new(err, isl_rs_ctx.last_error_msg()));
        }
        Ok(isl_rs_result)
    }

    /// Wraps `isl_multi_aff_from_range`.
    pub fn from_range(self) -> Result<MultiAff, LibISLError> {
        let multi = self;
        let isl_rs_ctx = multi.get_ctx();
        let mut multi = multi;
        multi.do_not_free_on_drop();
        let multi = multi.ptr;
        let isl_rs_result = unsafe { isl_multi_aff_from_range(multi) };
        let isl_rs_result = MultiAff { ptr: isl_rs_result,
                                       should_free_on_drop: true };
        let err = isl_rs_ctx.last_error();
        if err != Error::None_ {
            return Err(LibISLError::new(err, isl_rs_ctx.last_error_msg()));
        }
        Ok(isl_rs_result)
    }

    /// Wraps `isl_multi_aff_get_aff`.
    pub fn get_aff(&self, pos: i32) -> Result<Aff, LibISLError> {
        let multi = self;
        let isl_rs_ctx = multi.get_ctx();
        let multi = multi.ptr;
        let isl_rs_result = unsafe { isl_multi_aff_get_aff(multi, pos) };
        let isl_rs_result = Aff { ptr: isl_rs_result,
                                  should_free_on_drop: true };
        let err = isl_rs_ctx.last_error();
        if err != Error::None_ {
            return Err(LibISLError::new(err, isl_rs_ctx.last_error_msg()));
        }
        Ok(isl_rs_result)
    }

    /// Wraps `isl_multi_aff_get_at`.
    pub fn get_at(&self, pos: i32) -> Result<Aff, LibISLError> {
        let multi = self;
        let isl_rs_ctx = multi.get_ctx();
        let multi = multi.ptr;
        let isl_rs_result = unsafe { isl_multi_aff_get_at(multi, pos) };
        let isl_rs_result = Aff { ptr: isl_rs_result,
                                  should_free_on_drop: true };
        let err = isl_rs_ctx.last_error();
        if err != Error::None_ {
            return Err(LibISLError::new(err, isl_rs_ctx.last_error_msg()));
        }
        Ok(isl_rs_result)
    }

    /// Wraps `isl_multi_aff_get_constant_multi_val`.
    pub fn get_constant_multi_val(&self) -> Result<MultiVal, LibISLError> {
        let ma = self;
        let isl_rs_ctx = ma.get_ctx();
        let ma = ma.ptr;
        let isl_rs_result = unsafe { isl_multi_aff_get_constant_multi_val(ma) };
        let isl_rs_result = MultiVal { ptr: isl_rs_result,
                                       should_free_on_drop: true };
        let err = isl_rs_ctx.last_error();
        if err != Error::None_ {
            return Err(LibISLError::new(err, isl_rs_ctx.last_error_msg()));
        }
        Ok(isl_rs_result)
    }

    /// Wraps `isl_multi_aff_get_ctx`.
    pub fn get_ctx(&self) -> Context {
        let multi = self;
        let multi = multi.ptr;
        let isl_rs_result = unsafe { isl_multi_aff_get_ctx(multi) };
        let isl_rs_result = Context { ptr: isl_rs_result,
                                      should_free_on_drop: false };
        isl_rs_result
    }

    /// Wraps `isl_multi_aff_get_dim_id`.
    pub fn get_dim_id(&self, type_: DimType, pos: u32) -> Result<Id, LibISLError> {
        let multi = self;
        let isl_rs_ctx = multi.get_ctx();
        let multi = multi.ptr;
        let type_ = type_.to_i32();
        let isl_rs_result = unsafe { isl_multi_aff_get_dim_id(multi, type_, pos) };
        let isl_rs_result = Id { ptr: isl_rs_result,
                                 should_free_on_drop: true };
        let err = isl_rs_ctx.last_error();
        if err != Error::None_ {
            return Err(LibISLError::new(err, isl_rs_ctx.last_error_msg()));
        }
        Ok(isl_rs_result)
    }

    /// Wraps `isl_multi_aff_get_domain_space`.
    pub fn get_domain_space(&self) -> Result<Space, LibISLError> {
        let multi = self;
        let isl_rs_ctx = multi.get_ctx();
        let multi = multi.ptr;
        let isl_rs_result = unsafe { isl_multi_aff_get_domain_space(multi) };
        let isl_rs_result = Space { ptr: isl_rs_result,
                                    should_free_on_drop: true };
        let err = isl_rs_ctx.last_error();
        if err != Error::None_ {
            return Err(LibISLError::new(err, isl_rs_ctx.last_error_msg()));
        }
        Ok(isl_rs_result)
    }

    /// Wraps `isl_multi_aff_get_list`.
    pub fn get_list(&self) -> Result<AffList, LibISLError> {
        let multi = self;
        let isl_rs_ctx = multi.get_ctx();
        let multi = multi.ptr;
        let isl_rs_result = unsafe { isl_multi_aff_get_list(multi) };
        let isl_rs_result = AffList { ptr: isl_rs_result,
                                      should_free_on_drop: true };
        let err = isl_rs_ctx.last_error();
        if err != Error::None_ {
            return Err(LibISLError::new(err, isl_rs_ctx.last_error_msg()));
        }
        Ok(isl_rs_result)
    }

    /// Wraps `isl_multi_aff_get_range_tuple_id`.
    pub fn get_range_tuple_id(&self) -> Result<Id, LibISLError> {
        let multi = self;
        let isl_rs_ctx = multi.get_ctx();
        let multi = multi.ptr;
        let isl_rs_result = unsafe { isl_multi_aff_get_range_tuple_id(multi) };
        let isl_rs_result = Id { ptr: isl_rs_result,
                                 should_free_on_drop: true };
        let err = isl_rs_ctx.last_error();
        if err != Error::None_ {
            return Err(LibISLError::new(err, isl_rs_ctx.last_error_msg()));
        }
        Ok(isl_rs_result)
    }

    /// Wraps `isl_multi_aff_get_space`.
    pub fn get_space(&self) -> Result<Space, LibISLError> {
        let multi = self;
        let isl_rs_ctx = multi.get_ctx();
        let multi = multi.ptr;
        let isl_rs_result = unsafe { isl_multi_aff_get_space(multi) };
        let isl_rs_result = Space { ptr: isl_rs_result,
                                    should_free_on_drop: true };
        let err = isl_rs_ctx.last_error();
        if err != Error::None_ {
            return Err(LibISLError::new(err, isl_rs_ctx.last_error_msg()));
        }
        Ok(isl_rs_result)
    }

    /// Wraps `isl_multi_aff_get_tuple_id`.
    pub fn get_tuple_id(&self, type_: DimType) -> Result<Id, LibISLError> {
        let multi = self;
        let isl_rs_ctx = multi.get_ctx();
        let multi = multi.ptr;
        let type_ = type_.to_i32();
        let isl_rs_result = unsafe { isl_multi_aff_get_tuple_id(multi, type_) };
        let isl_rs_result = Id { ptr: isl_rs_result,
                                 should_free_on_drop: true };
        let err = isl_rs_ctx.last_error();
        if err != Error::None_ {
            return Err(LibISLError::new(err, isl_rs_ctx.last_error_msg()));
        }
        Ok(isl_rs_result)
    }

    /// Wraps `isl_multi_aff_get_tuple_name`.
    pub fn get_tuple_name(&self, type_: DimType) -> Result<&str, LibISLError> {
        let multi = self;
        let isl_rs_ctx = multi.get_ctx();
        let multi = multi.ptr;
        let type_ = type_.to_i32();
        let isl_rs_result = unsafe { isl_multi_aff_get_tuple_name(multi, type_) };
        let isl_rs_result = unsafe { CStr::from_ptr(isl_rs_result) };
        let isl_rs_result = isl_rs_result.to_str().unwrap();
        let err = isl_rs_ctx.last_error();
        if err != Error::None_ {
            return Err(LibISLError::new(err, isl_rs_ctx.last_error_msg()));
        }
        Ok(isl_rs_result)
    }

    /// Wraps `isl_multi_aff_gist`.
    pub fn gist(self, context: Set) -> Result<MultiAff, LibISLError> {
        let maff = self;
        let isl_rs_ctx = maff.get_ctx();
        let mut maff = maff;
        maff.do_not_free_on_drop();
        let maff = maff.ptr;
        let mut context = context;
        context.do_not_free_on_drop();
        let context = context.ptr;
        let isl_rs_result = unsafe { isl_multi_aff_gist(maff, context) };
        let isl_rs_result = MultiAff { ptr: isl_rs_result,
                                       should_free_on_drop: true };
        let err = isl_rs_ctx.last_error();
        if err != Error::None_ {
            return Err(LibISLError::new(err, isl_rs_ctx.last_error_msg()));
        }
        Ok(isl_rs_result)
    }

    /// Wraps `isl_multi_aff_gist_params`.
    pub fn gist_params(self, context: Set) -> Result<MultiAff, LibISLError> {
        let maff = self;
        let isl_rs_ctx = maff.get_ctx();
        let mut maff = maff;
        maff.do_not_free_on_drop();
        let maff = maff.ptr;
        let mut context = context;
        context.do_not_free_on_drop();
        let context = context.ptr;
        let isl_rs_result = unsafe { isl_multi_aff_gist_params(maff, context) };
        let isl_rs_result = MultiAff { ptr: isl_rs_result,
                                       should_free_on_drop: true };
        let err = isl_rs_ctx.last_error();
        if err != Error::None_ {
            return Err(LibISLError::new(err, isl_rs_ctx.last_error_msg()));
        }
        Ok(isl_rs_result)
    }

    /// Wraps `isl_multi_aff_has_range_tuple_id`.
    pub fn has_range_tuple_id(&self) -> Result<bool, LibISLError> {
        let multi = self;
        let isl_rs_ctx = multi.get_ctx();
        let multi = multi.ptr;
        let isl_rs_result = unsafe { isl_multi_aff_has_range_tuple_id(multi) };
        let isl_rs_result = match isl_rs_result {
            0 => false,
            1 => true,
            _ => panic!("Got isl_bool = -1"),
        };
        let err = isl_rs_ctx.last_error();
        if err != Error::None_ {
            return Err(LibISLError::new(err, isl_rs_ctx.last_error_msg()));
        }
        Ok(isl_rs_result)
    }

    /// Wraps `isl_multi_aff_has_tuple_id`.
    pub fn has_tuple_id(&self, type_: DimType) -> Result<bool, LibISLError> {
        let multi = self;
        let isl_rs_ctx = multi.get_ctx();
        let multi = multi.ptr;
        let type_ = type_.to_i32();
        let isl_rs_result = unsafe { isl_multi_aff_has_tuple_id(multi, type_) };
        let isl_rs_result = match isl_rs_result {
            0 => false,
            1 => true,
            _ => panic!("Got isl_bool = -1"),
        };
        let err = isl_rs_ctx.last_error();
        if err != Error::None_ {
            return Err(LibISLError::new(err, isl_rs_ctx.last_error_msg()));
        }
        Ok(isl_rs_result)
    }

    /// Wraps `isl_multi_aff_identity`.
    pub fn identity(space: Space) -> Result<MultiAff, LibISLError> {
        let isl_rs_ctx = space.get_ctx();
        let mut space = space;
        space.do_not_free_on_drop();
        let space = space.ptr;
        let isl_rs_result = unsafe { isl_multi_aff_identity(space) };
        let isl_rs_result = MultiAff { ptr: isl_rs_result,
                                       should_free_on_drop: true };
        let err = isl_rs_ctx.last_error();
        if err != Error::None_ {
            return Err(LibISLError::new(err, isl_rs_ctx.last_error_msg()));
        }
        Ok(isl_rs_result)
    }

    /// Wraps `isl_multi_aff_identity_multi_aff`.
    pub fn identity_multi_aff(self) -> Result<MultiAff, LibISLError> {
        let multi = self;
        let isl_rs_ctx = multi.get_ctx();
        let mut multi = multi;
        multi.do_not_free_on_drop();
        let multi = multi.ptr;
        let isl_rs_result = unsafe { isl_multi_aff_identity_multi_aff(multi) };
        let isl_rs_result = MultiAff { ptr: isl_rs_result,
                                       should_free_on_drop: true };
        let err = isl_rs_ctx.last_error();
        if err != Error::None_ {
            return Err(LibISLError::new(err, isl_rs_ctx.last_error_msg()));
        }
        Ok(isl_rs_result)
    }

    /// Wraps `isl_multi_aff_identity_on_domain_space`.
    pub fn identity_on_domain_space(space: Space) -> Result<MultiAff, LibISLError> {
        let isl_rs_ctx = space.get_ctx();
        let mut space = space;
        space.do_not_free_on_drop();
        let space = space.ptr;
        let isl_rs_result = unsafe { isl_multi_aff_identity_on_domain_space(space) };
        let isl_rs_result = MultiAff { ptr: isl_rs_result,
                                       should_free_on_drop: true };
        let err = isl_rs_ctx.last_error();
        if err != Error::None_ {
            return Err(LibISLError::new(err, isl_rs_ctx.last_error_msg()));
        }
        Ok(isl_rs_result)
    }

    /// Wraps `isl_multi_aff_insert_dims`.
    pub fn insert_dims(self, type_: DimType, first: u32, n: u32) -> Result<MultiAff, LibISLError> {
        let multi = self;
        let isl_rs_ctx = multi.get_ctx();
        let mut multi = multi;
        multi.do_not_free_on_drop();
        let multi = multi.ptr;
        let type_ = type_.to_i32();
        let isl_rs_result = unsafe { isl_multi_aff_insert_dims(multi, type_, first, n) };
        let isl_rs_result = MultiAff { ptr: isl_rs_result,
                                       should_free_on_drop: true };
        let err = isl_rs_ctx.last_error();
        if err != Error::None_ {
            return Err(LibISLError::new(err, isl_rs_ctx.last_error_msg()));
        }
        Ok(isl_rs_result)
    }

    /// Wraps `isl_multi_aff_insert_domain`.
    pub fn insert_domain(self, domain: Space) -> Result<MultiAff, LibISLError> {
        let multi = self;
        let isl_rs_ctx = multi.get_ctx();
        let mut multi = multi;
        multi.do_not_free_on_drop();
        let multi = multi.ptr;
        let mut domain = domain;
        domain.do_not_free_on_drop();
        let domain = domain.ptr;
        let isl_rs_result = unsafe { isl_multi_aff_insert_domain(multi, domain) };
        let isl_rs_result = MultiAff { ptr: isl_rs_result,
                                       should_free_on_drop: true };
        let err = isl_rs_ctx.last_error();
        if err != Error::None_ {
            return Err(LibISLError::new(err, isl_rs_ctx.last_error_msg()));
        }
        Ok(isl_rs_result)
    }

    /// Wraps `isl_multi_aff_involves_dims`.
    pub fn involves_dims(&self, type_: DimType, first: u32, n: u32) -> Result<bool, LibISLError> {
        let multi = self;
        let isl_rs_ctx = multi.get_ctx();
        let multi = multi.ptr;
        let type_ = type_.to_i32();
        let isl_rs_result = unsafe { isl_multi_aff_involves_dims(multi, type_, first, n) };
        let isl_rs_result = match isl_rs_result {
            0 => false,
            1 => true,
            _ => panic!("Got isl_bool = -1"),
        };
        let err = isl_rs_ctx.last_error();
        if err != Error::None_ {
            return Err(LibISLError::new(err, isl_rs_ctx.last_error_msg()));
        }
        Ok(isl_rs_result)
    }

    /// Wraps `isl_multi_aff_involves_locals`.
    pub fn involves_locals(&self) -> Result<bool, LibISLError> {
        let multi = self;
        let isl_rs_ctx = multi.get_ctx();
        let multi = multi.ptr;
        let isl_rs_result = unsafe { isl_multi_aff_involves_locals(multi) };
        let isl_rs_result = match isl_rs_result {
            0 => false,
            1 => true,
            _ => panic!("Got isl_bool = -1"),
        };
        let err = isl_rs_ctx.last_error();
        if err != Error::None_ {
            return Err(LibISLError::new(err, isl_rs_ctx.last_error_msg()));
        }
        Ok(isl_rs_result)
    }

    /// Wraps `isl_multi_aff_involves_nan`.
    pub fn involves_nan(&self) -> Result<bool, LibISLError> {
        let multi = self;
        let isl_rs_ctx = multi.get_ctx();
        let multi = multi.ptr;
        let isl_rs_result = unsafe { isl_multi_aff_involves_nan(multi) };
        let isl_rs_result = match isl_rs_result {
            0 => false,
            1 => true,
            _ => panic!("Got isl_bool = -1"),
        };
        let err = isl_rs_ctx.last_error();
        if err != Error::None_ {
            return Err(LibISLError::new(err, isl_rs_ctx.last_error_msg()));
        }
        Ok(isl_rs_result)
    }

    /// Wraps `isl_multi_aff_lex_ge_set`.
    pub fn lex_ge_set(self, ma2: MultiAff) -> Result<Set, LibISLError> {
        let ma1 = self;
        let isl_rs_ctx = ma1.get_ctx();
        let mut ma1 = ma1;
        ma1.do_not_free_on_drop();
        let ma1 = ma1.ptr;
        let mut ma2 = ma2;
        ma2.do_not_free_on_drop();
        let ma2 = ma2.ptr;
        let isl_rs_result = unsafe { isl_multi_aff_lex_ge_set(ma1, ma2) };
        let isl_rs_result = Set { ptr: isl_rs_result,
                                  should_free_on_drop: true };
        let err = isl_rs_ctx.last_error();
        if err != Error::None_ {
            return Err(LibISLError::new(err, isl_rs_ctx.last_error_msg()));
        }
        Ok(isl_rs_result)
    }

    /// Wraps `isl_multi_aff_lex_gt_set`.
    pub fn lex_gt_set(self, ma2: MultiAff) -> Result<Set, LibISLError> {
        let ma1 = self;
        let isl_rs_ctx = ma1.get_ctx();
        let mut ma1 = ma1;
        ma1.do_not_free_on_drop();
        let ma1 = ma1.ptr;
        let mut ma2 = ma2;
        ma2.do_not_free_on_drop();
        let ma2 = ma2.ptr;
        let isl_rs_result = unsafe { isl_multi_aff_lex_gt_set(ma1, ma2) };
        let isl_rs_result = Set { ptr: isl_rs_result,
                                  should_free_on_drop: true };
        let err = isl_rs_ctx.last_error();
        if err != Error::None_ {
            return Err(LibISLError::new(err, isl_rs_ctx.last_error_msg()));
        }
        Ok(isl_rs_result)
    }

    /// Wraps `isl_multi_aff_lex_le_set`.
    pub fn lex_le_set(self, ma2: MultiAff) -> Result<Set, LibISLError> {
        let ma1 = self;
        let isl_rs_ctx = ma1.get_ctx();
        let mut ma1 = ma1;
        ma1.do_not_free_on_drop();
        let ma1 = ma1.ptr;
        let mut ma2 = ma2;
        ma2.do_not_free_on_drop();
        let ma2 = ma2.ptr;
        let isl_rs_result = unsafe { isl_multi_aff_lex_le_set(ma1, ma2) };
        let isl_rs_result = Set { ptr: isl_rs_result,
                                  should_free_on_drop: true };
        let err = isl_rs_ctx.last_error();
        if err != Error::None_ {
            return Err(LibISLError::new(err, isl_rs_ctx.last_error_msg()));
        }
        Ok(isl_rs_result)
    }

    /// Wraps `isl_multi_aff_lex_lt_set`.
    pub fn lex_lt_set(self, ma2: MultiAff) -> Result<Set, LibISLError> {
        let ma1 = self;
        let isl_rs_ctx = ma1.get_ctx();
        let mut ma1 = ma1;
        ma1.do_not_free_on_drop();
        let ma1 = ma1.ptr;
        let mut ma2 = ma2;
        ma2.do_not_free_on_drop();
        let ma2 = ma2.ptr;
        let isl_rs_result = unsafe { isl_multi_aff_lex_lt_set(ma1, ma2) };
        let isl_rs_result = Set { ptr: isl_rs_result,
                                  should_free_on_drop: true };
        let err = isl_rs_ctx.last_error();
        if err != Error::None_ {
            return Err(LibISLError::new(err, isl_rs_ctx.last_error_msg()));
        }
        Ok(isl_rs_result)
    }

    /// Wraps `isl_multi_aff_mod_multi_val`.
    pub fn mod_multi_val(self, mv: MultiVal) -> Result<MultiAff, LibISLError> {
        let multi = self;
        let isl_rs_ctx = multi.get_ctx();
        let mut multi = multi;
        multi.do_not_free_on_drop();
        let multi = multi.ptr;
        let mut mv = mv;
        mv.do_not_free_on_drop();
        let mv = mv.ptr;
        let isl_rs_result = unsafe { isl_multi_aff_mod_multi_val(multi, mv) };
        let isl_rs_result = MultiAff { ptr: isl_rs_result,
                                       should_free_on_drop: true };
        let err = isl_rs_ctx.last_error();
        if err != Error::None_ {
            return Err(LibISLError::new(err, isl_rs_ctx.last_error_msg()));
        }
        Ok(isl_rs_result)
    }

    /// Wraps `isl_multi_aff_move_dims`.
    pub fn move_dims(self, dst_type: DimType, dst_pos: u32, src_type: DimType, src_pos: u32,
                     n: u32)
                     -> Result<MultiAff, LibISLError> {
        let ma = self;
        let isl_rs_ctx = ma.get_ctx();
        let mut ma = ma;
        ma.do_not_free_on_drop();
        let ma = ma.ptr;
        let dst_type = dst_type.to_i32();
        let src_type = src_type.to_i32();
        let isl_rs_result =
            unsafe { isl_multi_aff_move_dims(ma, dst_type, dst_pos, src_type, src_pos, n) };
        let isl_rs_result = MultiAff { ptr: isl_rs_result,
                                       should_free_on_drop: true };
        let err = isl_rs_ctx.last_error();
        if err != Error::None_ {
            return Err(LibISLError::new(err, isl_rs_ctx.last_error_msg()));
        }
        Ok(isl_rs_result)
    }

    /// Wraps `isl_multi_aff_multi_val_on_domain_space`.
    pub fn multi_val_on_domain_space(space: Space, mv: MultiVal) -> Result<MultiAff, LibISLError> {
        let isl_rs_ctx = space.get_ctx();
        let mut space = space;
        space.do_not_free_on_drop();
        let space = space.ptr;
        let mut mv = mv;
        mv.do_not_free_on_drop();
        let mv = mv.ptr;
        let isl_rs_result = unsafe { isl_multi_aff_multi_val_on_domain_space(space, mv) };
        let isl_rs_result = MultiAff { ptr: isl_rs_result,
                                       should_free_on_drop: true };
        let err = isl_rs_ctx.last_error();
        if err != Error::None_ {
            return Err(LibISLError::new(err, isl_rs_ctx.last_error_msg()));
        }
        Ok(isl_rs_result)
    }

    /// Wraps `isl_multi_aff_multi_val_on_space`.
    pub fn multi_val_on_space(space: Space, mv: MultiVal) -> Result<MultiAff, LibISLError> {
        let isl_rs_ctx = space.get_ctx();
        let mut space = space;
        space.do_not_free_on_drop();
        let space = space.ptr;
        let mut mv = mv;
        mv.do_not_free_on_drop();
        let mv = mv.ptr;
        let isl_rs_result = unsafe { isl_multi_aff_multi_val_on_space(space, mv) };
        let isl_rs_result = MultiAff { ptr: isl_rs_result,
                                       should_free_on_drop: true };
        let err = isl_rs_ctx.last_error();
        if err != Error::None_ {
            return Err(LibISLError::new(err, isl_rs_ctx.last_error_msg()));
        }
        Ok(isl_rs_result)
    }

    /// Wraps `isl_multi_aff_neg`.
    pub fn neg(self) -> Result<MultiAff, LibISLError> {
        let multi = self;
        let isl_rs_ctx = multi.get_ctx();
        let mut multi = multi;
        multi.do_not_free_on_drop();
        let multi = multi.ptr;
        let isl_rs_result = unsafe { isl_multi_aff_neg(multi) };
        let isl_rs_result = MultiAff { ptr: isl_rs_result,
                                       should_free_on_drop: true };
        let err = isl_rs_ctx.last_error();
        if err != Error::None_ {
            return Err(LibISLError::new(err, isl_rs_ctx.last_error_msg()));
        }
        Ok(isl_rs_result)
    }

    /// Wraps `isl_multi_aff_plain_cmp`.
    pub fn plain_cmp(&self, multi2: &MultiAff) -> Result<i32, LibISLError> {
        let multi1 = self;
        let isl_rs_ctx = multi1.get_ctx();
        let multi1 = multi1.ptr;
        let multi2 = multi2.ptr;
        let isl_rs_result = unsafe { isl_multi_aff_plain_cmp(multi1, multi2) };
        let err = isl_rs_ctx.last_error();
        if err != Error::None_ {
            return Err(LibISLError::new(err, isl_rs_ctx.last_error_msg()));
        }
        Ok(isl_rs_result)
    }

    /// Wraps `isl_multi_aff_plain_is_equal`.
    pub fn plain_is_equal(&self, multi2: &MultiAff) -> Result<bool, LibISLError> {
        let multi1 = self;
        let isl_rs_ctx = multi1.get_ctx();
        let multi1 = multi1.ptr;
        let multi2 = multi2.ptr;
        let isl_rs_result = unsafe { isl_multi_aff_plain_is_equal(multi1, multi2) };
        let isl_rs_result = match isl_rs_result {
            0 => false,
            1 => true,
            _ => panic!("Got isl_bool = -1"),
        };
        let err = isl_rs_ctx.last_error();
        if err != Error::None_ {
            return Err(LibISLError::new(err, isl_rs_ctx.last_error_msg()));
        }
        Ok(isl_rs_result)
    }

    /// Wraps `isl_multi_aff_product`.
    pub fn product(self, multi2: MultiAff) -> Result<MultiAff, LibISLError> {
        let multi1 = self;
        let isl_rs_ctx = multi1.get_ctx();
        let mut multi1 = multi1;
        multi1.do_not_free_on_drop();
        let multi1 = multi1.ptr;
        let mut multi2 = multi2;
        multi2.do_not_free_on_drop();
        let multi2 = multi2.ptr;
        let isl_rs_result = unsafe { isl_multi_aff_product(multi1, multi2) };
        let isl_rs_result = MultiAff { ptr: isl_rs_result,
                                       should_free_on_drop: true };
        let err = isl_rs_ctx.last_error();
        if err != Error::None_ {
            return Err(LibISLError::new(err, isl_rs_ctx.last_error_msg()));
        }
        Ok(isl_rs_result)
    }

    /// Wraps `isl_multi_aff_project_domain_on_params`.
    pub fn project_domain_on_params(self) -> Result<MultiAff, LibISLError> {
        let multi = self;
        let isl_rs_ctx = multi.get_ctx();
        let mut multi = multi;
        multi.do_not_free_on_drop();
        let multi = multi.ptr;
        let isl_rs_result = unsafe { isl_multi_aff_project_domain_on_params(multi) };
        let isl_rs_result = MultiAff { ptr: isl_rs_result,
                                       should_free_on_drop: true };
        let err = isl_rs_ctx.last_error();
        if err != Error::None_ {
            return Err(LibISLError::new(err, isl_rs_ctx.last_error_msg()));
        }
        Ok(isl_rs_result)
    }

    /// Wraps `isl_multi_aff_project_out_map`.
    pub fn project_out_map(space: Space, type_: DimType, first: u32, n: u32)
                           -> Result<MultiAff, LibISLError> {
        let isl_rs_ctx = space.get_ctx();
        let mut space = space;
        space.do_not_free_on_drop();
        let space = space.ptr;
        let type_ = type_.to_i32();
        let isl_rs_result = unsafe { isl_multi_aff_project_out_map(space, type_, first, n) };
        let isl_rs_result = MultiAff { ptr: isl_rs_result,
                                       should_free_on_drop: true };
        let err = isl_rs_ctx.last_error();
        if err != Error::None_ {
            return Err(LibISLError::new(err, isl_rs_ctx.last_error_msg()));
        }
        Ok(isl_rs_result)
    }

    /// Wraps `isl_multi_aff_pullback_multi_aff`.
    pub fn pullback_multi_aff(self, ma2: MultiAff) -> Result<MultiAff, LibISLError> {
        let ma1 = self;
        let isl_rs_ctx = ma1.get_ctx();
        let mut ma1 = ma1;
        ma1.do_not_free_on_drop();
        let ma1 = ma1.ptr;
        let mut ma2 = ma2;
        ma2.do_not_free_on_drop();
        let ma2 = ma2.ptr;
        let isl_rs_result = unsafe { isl_multi_aff_pullback_multi_aff(ma1, ma2) };
        let isl_rs_result = MultiAff { ptr: isl_rs_result,
                                       should_free_on_drop: true };
        let err = isl_rs_ctx.last_error();
        if err != Error::None_ {
            return Err(LibISLError::new(err, isl_rs_ctx.last_error_msg()));
        }
        Ok(isl_rs_result)
    }

    /// Wraps `isl_multi_aff_range_factor_domain`.
    pub fn range_factor_domain(self) -> Result<MultiAff, LibISLError> {
        let multi = self;
        let isl_rs_ctx = multi.get_ctx();
        let mut multi = multi;
        multi.do_not_free_on_drop();
        let multi = multi.ptr;
        let isl_rs_result = unsafe { isl_multi_aff_range_factor_domain(multi) };
        let isl_rs_result = MultiAff { ptr: isl_rs_result,
                                       should_free_on_drop: true };
        let err = isl_rs_ctx.last_error();
        if err != Error::None_ {
            return Err(LibISLError::new(err, isl_rs_ctx.last_error_msg()));
        }
        Ok(isl_rs_result)
    }

    /// Wraps `isl_multi_aff_range_factor_range`.
    pub fn range_factor_range(self) -> Result<MultiAff, LibISLError> {
        let multi = self;
        let isl_rs_ctx = multi.get_ctx();
        let mut multi = multi;
        multi.do_not_free_on_drop();
        let multi = multi.ptr;
        let isl_rs_result = unsafe { isl_multi_aff_range_factor_range(multi) };
        let isl_rs_result = MultiAff { ptr: isl_rs_result,
                                       should_free_on_drop: true };
        let err = isl_rs_ctx.last_error();
        if err != Error::None_ {
            return Err(LibISLError::new(err, isl_rs_ctx.last_error_msg()));
        }
        Ok(isl_rs_result)
    }

    /// Wraps `isl_multi_aff_range_is_wrapping`.
    pub fn range_is_wrapping(&self) -> Result<bool, LibISLError> {
        let multi = self;
        let isl_rs_ctx = multi.get_ctx();
        let multi = multi.ptr;
        let isl_rs_result = unsafe { isl_multi_aff_range_is_wrapping(multi) };
        let isl_rs_result = match isl_rs_result {
            0 => false,
            1 => true,
            _ => panic!("Got isl_bool = -1"),
        };
        let err = isl_rs_ctx.last_error();
        if err != Error::None_ {
            return Err(LibISLError::new(err, isl_rs_ctx.last_error_msg()));
        }
        Ok(isl_rs_result)
    }

    /// Wraps `isl_multi_aff_range_map`.
    pub fn range_map(space: Space) -> Result<MultiAff, LibISLError> {
        let isl_rs_ctx = space.get_ctx();
        let mut space = space;
        space.do_not_free_on_drop();
        let space = space.ptr;
        let isl_rs_result = unsafe { isl_multi_aff_range_map(space) };
        let isl_rs_result = MultiAff { ptr: isl_rs_result,
                                       should_free_on_drop: true };
        let err = isl_rs_ctx.last_error();
        if err != Error::None_ {
            return Err(LibISLError::new(err, isl_rs_ctx.last_error_msg()));
        }
        Ok(isl_rs_result)
    }

    /// Wraps `isl_multi_aff_range_product`.
    pub fn range_product(self, multi2: MultiAff) -> Result<MultiAff, LibISLError> {
        let multi1 = self;
        let isl_rs_ctx = multi1.get_ctx();
        let mut multi1 = multi1;
        multi1.do_not_free_on_drop();
        let multi1 = multi1.ptr;
        let mut multi2 = multi2;
        multi2.do_not_free_on_drop();
        let multi2 = multi2.ptr;
        let isl_rs_result = unsafe { isl_multi_aff_range_product(multi1, multi2) };
        let isl_rs_result = MultiAff { ptr: isl_rs_result,
                                       should_free_on_drop: true };
        let err = isl_rs_ctx.last_error();
        if err != Error::None_ {
            return Err(LibISLError::new(err, isl_rs_ctx.last_error_msg()));
        }
        Ok(isl_rs_result)
    }

    /// Wraps `isl_multi_aff_range_splice`.
    pub fn range_splice(self, pos: u32, multi2: MultiAff) -> Result<MultiAff, LibISLError> {
        let multi1 = self;
        let isl_rs_ctx = multi1.get_ctx();
        let mut multi1 = multi1;
        multi1.do_not_free_on_drop();
        let multi1 = multi1.ptr;
        let mut multi2 = multi2;
        multi2.do_not_free_on_drop();
        let multi2 = multi2.ptr;
        let isl_rs_result = unsafe { isl_multi_aff_range_splice(multi1, pos, multi2) };
        let isl_rs_result = MultiAff { ptr: isl_rs_result,
                                       should_free_on_drop: true };
        let err = isl_rs_ctx.last_error();
        if err != Error::None_ {
            return Err(LibISLError::new(err, isl_rs_ctx.last_error_msg()));
        }
        Ok(isl_rs_result)
    }

    /// Wraps `isl_multi_aff_read_from_str`.
    pub fn read_from_str(ctx: &Context, str_: &str) -> Result<MultiAff, LibISLError> {
        let isl_rs_ctx = Context { ptr: ctx.ptr,
                                   should_free_on_drop: false };
        let ctx = ctx.ptr;
        let str_ = CString::new(str_).unwrap();
        let str_ = str_.as_ptr();
        let isl_rs_result = unsafe { isl_multi_aff_read_from_str(ctx, str_) };
        let isl_rs_result = MultiAff { ptr: isl_rs_result,
                                       should_free_on_drop: true };
        let err = isl_rs_ctx.last_error();
        if err != Error::None_ {
            return Err(LibISLError::new(err, isl_rs_ctx.last_error_msg()));
        }
        Ok(isl_rs_result)
    }

    /// Wraps `isl_multi_aff_reset_range_tuple_id`.
    pub fn reset_range_tuple_id(self) -> Result<MultiAff, LibISLError> {
        let multi = self;
        let isl_rs_ctx = multi.get_ctx();
        let mut multi = multi;
        multi.do_not_free_on_drop();
        let multi = multi.ptr;
        let isl_rs_result = unsafe { isl_multi_aff_reset_range_tuple_id(multi) };
        let isl_rs_result = MultiAff { ptr: isl_rs_result,
                                       should_free_on_drop: true };
        let err = isl_rs_ctx.last_error();
        if err != Error::None_ {
            return Err(LibISLError::new(err, isl_rs_ctx.last_error_msg()));
        }
        Ok(isl_rs_result)
    }

    /// Wraps `isl_multi_aff_reset_tuple_id`.
    pub fn reset_tuple_id(self, type_: DimType) -> Result<MultiAff, LibISLError> {
        let multi = self;
        let isl_rs_ctx = multi.get_ctx();
        let mut multi = multi;
        multi.do_not_free_on_drop();
        let multi = multi.ptr;
        let type_ = type_.to_i32();
        let isl_rs_result = unsafe { isl_multi_aff_reset_tuple_id(multi, type_) };
        let isl_rs_result = MultiAff { ptr: isl_rs_result,
                                       should_free_on_drop: true };
        let err = isl_rs_ctx.last_error();
        if err != Error::None_ {
            return Err(LibISLError::new(err, isl_rs_ctx.last_error_msg()));
        }
        Ok(isl_rs_result)
    }

    /// Wraps `isl_multi_aff_reset_user`.
    pub fn reset_user(self) -> Result<MultiAff, LibISLError> {
        let multi = self;
        let isl_rs_ctx = multi.get_ctx();
        let mut multi = multi;
        multi.do_not_free_on_drop();
        let multi = multi.ptr;
        let isl_rs_result = unsafe { isl_multi_aff_reset_user(multi) };
        let isl_rs_result = MultiAff { ptr: isl_rs_result,
                                       should_free_on_drop: true };
        let err = isl_rs_ctx.last_error();
        if err != Error::None_ {
            return Err(LibISLError::new(err, isl_rs_ctx.last_error_msg()));
        }
        Ok(isl_rs_result)
    }

    /// Wraps `isl_multi_aff_scale_down_multi_val`.
    pub fn scale_down_multi_val(self, mv: MultiVal) -> Result<MultiAff, LibISLError> {
        let multi = self;
        let isl_rs_ctx = multi.get_ctx();
        let mut multi = multi;
        multi.do_not_free_on_drop();
        let multi = multi.ptr;
        let mut mv = mv;
        mv.do_not_free_on_drop();
        let mv = mv.ptr;
        let isl_rs_result = unsafe { isl_multi_aff_scale_down_multi_val(multi, mv) };
        let isl_rs_result = MultiAff { ptr: isl_rs_result,
                                       should_free_on_drop: true };
        let err = isl_rs_ctx.last_error();
        if err != Error::None_ {
            return Err(LibISLError::new(err, isl_rs_ctx.last_error_msg()));
        }
        Ok(isl_rs_result)
    }

    /// Wraps `isl_multi_aff_scale_down_val`.
    pub fn scale_down_val(self, v: Val) -> Result<MultiAff, LibISLError> {
        let multi = self;
        let isl_rs_ctx = multi.get_ctx();
        let mut multi = multi;
        multi.do_not_free_on_drop();
        let multi = multi.ptr;
        let mut v = v;
        v.do_not_free_on_drop();
        let v = v.ptr;
        let isl_rs_result = unsafe { isl_multi_aff_scale_down_val(multi, v) };
        let isl_rs_result = MultiAff { ptr: isl_rs_result,
                                       should_free_on_drop: true };
        let err = isl_rs_ctx.last_error();
        if err != Error::None_ {
            return Err(LibISLError::new(err, isl_rs_ctx.last_error_msg()));
        }
        Ok(isl_rs_result)
    }

    /// Wraps `isl_multi_aff_scale_multi_val`.
    pub fn scale_multi_val(self, mv: MultiVal) -> Result<MultiAff, LibISLError> {
        let multi = self;
        let isl_rs_ctx = multi.get_ctx();
        let mut multi = multi;
        multi.do_not_free_on_drop();
        let multi = multi.ptr;
        let mut mv = mv;
        mv.do_not_free_on_drop();
        let mv = mv.ptr;
        let isl_rs_result = unsafe { isl_multi_aff_scale_multi_val(multi, mv) };
        let isl_rs_result = MultiAff { ptr: isl_rs_result,
                                       should_free_on_drop: true };
        let err = isl_rs_ctx.last_error();
        if err != Error::None_ {
            return Err(LibISLError::new(err, isl_rs_ctx.last_error_msg()));
        }
        Ok(isl_rs_result)
    }

    /// Wraps `isl_multi_aff_scale_val`.
    pub fn scale_val(self, v: Val) -> Result<MultiAff, LibISLError> {
        let multi = self;
        let isl_rs_ctx = multi.get_ctx();
        let mut multi = multi;
        multi.do_not_free_on_drop();
        let multi = multi.ptr;
        let mut v = v;
        v.do_not_free_on_drop();
        let v = v.ptr;
        let isl_rs_result = unsafe { isl_multi_aff_scale_val(multi, v) };
        let isl_rs_result = MultiAff { ptr: isl_rs_result,
                                       should_free_on_drop: true };
        let err = isl_rs_ctx.last_error();
        if err != Error::None_ {
            return Err(LibISLError::new(err, isl_rs_ctx.last_error_msg()));
        }
        Ok(isl_rs_result)
    }

    /// Wraps `isl_multi_aff_set_aff`.
    pub fn set_aff(self, pos: i32, el: Aff) -> Result<MultiAff, LibISLError> {
        let multi = self;
        let isl_rs_ctx = multi.get_ctx();
        let mut multi = multi;
        multi.do_not_free_on_drop();
        let multi = multi.ptr;
        let mut el = el;
        el.do_not_free_on_drop();
        let el = el.ptr;
        let isl_rs_result = unsafe { isl_multi_aff_set_aff(multi, pos, el) };
        let isl_rs_result = MultiAff { ptr: isl_rs_result,
                                       should_free_on_drop: true };
        let err = isl_rs_ctx.last_error();
        if err != Error::None_ {
            return Err(LibISLError::new(err, isl_rs_ctx.last_error_msg()));
        }
        Ok(isl_rs_result)
    }

    /// Wraps `isl_multi_aff_set_at`.
    pub fn set_at(self, pos: i32, el: Aff) -> Result<MultiAff, LibISLError> {
        let multi = self;
        let isl_rs_ctx = multi.get_ctx();
        let mut multi = multi;
        multi.do_not_free_on_drop();
        let multi = multi.ptr;
        let mut el = el;
        el.do_not_free_on_drop();
        let el = el.ptr;
        let isl_rs_result = unsafe { isl_multi_aff_set_at(multi, pos, el) };
        let isl_rs_result = MultiAff { ptr: isl_rs_result,
                                       should_free_on_drop: true };
        let err = isl_rs_ctx.last_error();
        if err != Error::None_ {
            return Err(LibISLError::new(err, isl_rs_ctx.last_error_msg()));
        }
        Ok(isl_rs_result)
    }

    /// Wraps `isl_multi_aff_set_dim_id`.
    pub fn set_dim_id(self, type_: DimType, pos: u32, id: Id) -> Result<MultiAff, LibISLError> {
        let multi = self;
        let isl_rs_ctx = multi.get_ctx();
        let mut multi = multi;
        multi.do_not_free_on_drop();
        let multi = multi.ptr;
        let type_ = type_.to_i32();
        let mut id = id;
        id.do_not_free_on_drop();
        let id = id.ptr;
        let isl_rs_result = unsafe { isl_multi_aff_set_dim_id(multi, type_, pos, id) };
        let isl_rs_result = MultiAff { ptr: isl_rs_result,
                                       should_free_on_drop: true };
        let err = isl_rs_ctx.last_error();
        if err != Error::None_ {
            return Err(LibISLError::new(err, isl_rs_ctx.last_error_msg()));
        }
        Ok(isl_rs_result)
    }

    /// Wraps `isl_multi_aff_set_dim_name`.
    pub fn set_dim_name(self, type_: DimType, pos: u32, s: &str) -> Result<MultiAff, LibISLError> {
        let multi = self;
        let isl_rs_ctx = multi.get_ctx();
        let mut multi = multi;
        multi.do_not_free_on_drop();
        let multi = multi.ptr;
        let type_ = type_.to_i32();
        let s = CString::new(s).unwrap();
        let s = s.as_ptr();
        let isl_rs_result = unsafe { isl_multi_aff_set_dim_name(multi, type_, pos, s) };
        let isl_rs_result = MultiAff { ptr: isl_rs_result,
                                       should_free_on_drop: true };
        let err = isl_rs_ctx.last_error();
        if err != Error::None_ {
            return Err(LibISLError::new(err, isl_rs_ctx.last_error_msg()));
        }
        Ok(isl_rs_result)
    }

    /// Wraps `isl_multi_aff_set_range_tuple_id`.
    pub fn set_range_tuple_id(self, id: Id) -> Result<MultiAff, LibISLError> {
        let multi = self;
        let isl_rs_ctx = multi.get_ctx();
        let mut multi = multi;
        multi.do_not_free_on_drop();
        let multi = multi.ptr;
        let mut id = id;
        id.do_not_free_on_drop();
        let id = id.ptr;
        let isl_rs_result = unsafe { isl_multi_aff_set_range_tuple_id(multi, id) };
        let isl_rs_result = MultiAff { ptr: isl_rs_result,
                                       should_free_on_drop: true };
        let err = isl_rs_ctx.last_error();
        if err != Error::None_ {
            return Err(LibISLError::new(err, isl_rs_ctx.last_error_msg()));
        }
        Ok(isl_rs_result)
    }

    /// Wraps `isl_multi_aff_set_tuple_id`.
    pub fn set_tuple_id(self, type_: DimType, id: Id) -> Result<MultiAff, LibISLError> {
        let multi = self;
        let isl_rs_ctx = multi.get_ctx();
        let mut multi = multi;
        multi.do_not_free_on_drop();
        let multi = multi.ptr;
        let type_ = type_.to_i32();
        let mut id = id;
        id.do_not_free_on_drop();
        let id = id.ptr;
        let isl_rs_result = unsafe { isl_multi_aff_set_tuple_id(multi, type_, id) };
        let isl_rs_result = MultiAff { ptr: isl_rs_result,
                                       should_free_on_drop: true };
        let err = isl_rs_ctx.last_error();
        if err != Error::None_ {
            return Err(LibISLError::new(err, isl_rs_ctx.last_error_msg()));
        }
        Ok(isl_rs_result)
    }

    /// Wraps `isl_multi_aff_set_tuple_name`.
    pub fn set_tuple_name(self, type_: DimType, s: &str) -> Result<MultiAff, LibISLError> {
        let multi = self;
        let isl_rs_ctx = multi.get_ctx();
        let mut multi = multi;
        multi.do_not_free_on_drop();
        let multi = multi.ptr;
        let type_ = type_.to_i32();
        let s = CString::new(s).unwrap();
        let s = s.as_ptr();
        let isl_rs_result = unsafe { isl_multi_aff_set_tuple_name(multi, type_, s) };
        let isl_rs_result = MultiAff { ptr: isl_rs_result,
                                       should_free_on_drop: true };
        let err = isl_rs_ctx.last_error();
        if err != Error::None_ {
            return Err(LibISLError::new(err, isl_rs_ctx.last_error_msg()));
        }
        Ok(isl_rs_result)
    }

    /// Wraps `isl_multi_aff_size`.
    pub fn size(&self) -> Result<i32, LibISLError> {
        let multi = self;
        let isl_rs_ctx = multi.get_ctx();
        let multi = multi.ptr;
        let isl_rs_result = unsafe { isl_multi_aff_size(multi) };
        let err = isl_rs_ctx.last_error();
        if err != Error::None_ {
            return Err(LibISLError::new(err, isl_rs_ctx.last_error_msg()));
        }
        Ok(isl_rs_result)
    }

    /// Wraps `isl_multi_aff_splice`.
    pub fn splice(self, in_pos: u32, out_pos: u32, multi2: MultiAff)
                  -> Result<MultiAff, LibISLError> {
        let multi1 = self;
        let isl_rs_ctx = multi1.get_ctx();
        let mut multi1 = multi1;
        multi1.do_not_free_on_drop();
        let multi1 = multi1.ptr;
        let mut multi2 = multi2;
        multi2.do_not_free_on_drop();
        let multi2 = multi2.ptr;
        let isl_rs_result = unsafe { isl_multi_aff_splice(multi1, in_pos, out_pos, multi2) };
        let isl_rs_result = MultiAff { ptr: isl_rs_result,
                                       should_free_on_drop: true };
        let err = isl_rs_ctx.last_error();
        if err != Error::None_ {
            return Err(LibISLError::new(err, isl_rs_ctx.last_error_msg()));
        }
        Ok(isl_rs_result)
    }

    /// Wraps `isl_multi_aff_sub`.
    pub fn sub(self, multi2: MultiAff) -> Result<MultiAff, LibISLError> {
        let multi1 = self;
        let isl_rs_ctx = multi1.get_ctx();
        let mut multi1 = multi1;
        multi1.do_not_free_on_drop();
        let multi1 = multi1.ptr;
        let mut multi2 = multi2;
        multi2.do_not_free_on_drop();
        let multi2 = multi2.ptr;
        let isl_rs_result = unsafe { isl_multi_aff_sub(multi1, multi2) };
        let isl_rs_result = MultiAff { ptr: isl_rs_result,
                                       should_free_on_drop: true };
        let err = isl_rs_ctx.last_error();
        if err != Error::None_ {
            return Err(LibISLError::new(err, isl_rs_ctx.last_error_msg()));
        }
        Ok(isl_rs_result)
    }

    /// Wraps `isl_multi_aff_to_multi_pw_aff`.
    pub fn to_multi_pw_aff(self) -> Result<MultiPwAff, LibISLError> {
        let ma = self;
        let isl_rs_ctx = ma.get_ctx();
        let mut ma = ma;
        ma.do_not_free_on_drop();
        let ma = ma.ptr;
        let isl_rs_result = unsafe { isl_multi_aff_to_multi_pw_aff(ma) };
        let isl_rs_result = MultiPwAff { ptr: isl_rs_result,
                                         should_free_on_drop: true };
        let err = isl_rs_ctx.last_error();
        if err != Error::None_ {
            return Err(LibISLError::new(err, isl_rs_ctx.last_error_msg()));
        }
        Ok(isl_rs_result)
    }

    /// Wraps `isl_multi_aff_to_multi_union_pw_aff`.
    pub fn to_multi_union_pw_aff(self) -> Result<MultiUnionPwAff, LibISLError> {
        let ma = self;
        let isl_rs_ctx = ma.get_ctx();
        let mut ma = ma;
        ma.do_not_free_on_drop();
        let ma = ma.ptr;
        let isl_rs_result = unsafe { isl_multi_aff_to_multi_union_pw_aff(ma) };
        let isl_rs_result = MultiUnionPwAff { ptr: isl_rs_result,
                                              should_free_on_drop: true };
        let err = isl_rs_ctx.last_error();
        if err != Error::None_ {
            return Err(LibISLError::new(err, isl_rs_ctx.last_error_msg()));
        }
        Ok(isl_rs_result)
    }

    /// Wraps `isl_multi_aff_to_pw_multi_aff`.
    pub fn to_pw_multi_aff(self) -> Result<PwMultiAff, LibISLError> {
        let ma = self;
        let isl_rs_ctx = ma.get_ctx();
        let mut ma = ma;
        ma.do_not_free_on_drop();
        let ma = ma.ptr;
        let isl_rs_result = unsafe { isl_multi_aff_to_pw_multi_aff(ma) };
        let isl_rs_result = PwMultiAff { ptr: isl_rs_result,
                                         should_free_on_drop: true };
        let err = isl_rs_ctx.last_error();
        if err != Error::None_ {
            return Err(LibISLError::new(err, isl_rs_ctx.last_error_msg()));
        }
        Ok(isl_rs_result)
    }

    /// Wraps `isl_multi_aff_to_str`.
    pub fn to_str(&self) -> Result<&str, LibISLError> {
        let ma = self;
        let isl_rs_ctx = ma.get_ctx();
        let ma = ma.ptr;
        let isl_rs_result = unsafe { isl_multi_aff_to_str(ma) };
        let isl_rs_result = unsafe { CStr::from_ptr(isl_rs_result) };
        let isl_rs_result = isl_rs_result.to_str().unwrap();
        let err = isl_rs_ctx.last_error();
        if err != Error::None_ {
            return Err(LibISLError::new(err, isl_rs_ctx.last_error_msg()));
        }
        Ok(isl_rs_result)
    }

    /// Wraps `isl_multi_aff_unbind_params_insert_domain`.
    pub fn unbind_params_insert_domain(self, domain: MultiId) -> Result<MultiAff, LibISLError> {
        let multi = self;
        let isl_rs_ctx = multi.get_ctx();
        let mut multi = multi;
        multi.do_not_free_on_drop();
        let multi = multi.ptr;
        let mut domain = domain;
        domain.do_not_free_on_drop();
        let domain = domain.ptr;
        let isl_rs_result = unsafe { isl_multi_aff_unbind_params_insert_domain(multi, domain) };
        let isl_rs_result = MultiAff { ptr: isl_rs_result,
                                       should_free_on_drop: true };
        let err = isl_rs_ctx.last_error();
        if err != Error::None_ {
            return Err(LibISLError::new(err, isl_rs_ctx.last_error_msg()));
        }
        Ok(isl_rs_result)
    }

    /// Wraps `isl_multi_aff_zero`.
    pub fn zero(space: Space) -> Result<MultiAff, LibISLError> {
        let isl_rs_ctx = space.get_ctx();
        let mut space = space;
        space.do_not_free_on_drop();
        let space = space.ptr;
        let isl_rs_result = unsafe { isl_multi_aff_zero(space) };
        let isl_rs_result = MultiAff { ptr: isl_rs_result,
                                       should_free_on_drop: true };
        let err = isl_rs_ctx.last_error();
        if err != Error::None_ {
            return Err(LibISLError::new(err, isl_rs_ctx.last_error_msg()));
        }
        Ok(isl_rs_result)
    }

    /// Does not call isl_multi_aff_free() on being dropped. (For internal use
    /// only.)
    pub fn do_not_free_on_drop(&mut self) {
        self.should_free_on_drop = false;
    }
}

impl Drop for MultiAff {
    fn drop(&mut self) {
        if self.should_free_on_drop {
            unsafe {
                isl_multi_aff_free(self.ptr);
            }
        }
    }
}
