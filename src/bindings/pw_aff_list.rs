...
use super::{Context, Error, LibISLError, PwAff, Set};
use libc::uintptr_t;
...
impl PwAffList {
    ...
    pub fn add(self, el: PwAff) -> Result<PwAffList, LibISLError> {
        let list = self;
    ...
    pub fn get_at(&self, index: i32) -> Result<PwAff, LibISLError> {
        let list = self;
        let isl_rs_ctx = list.get_ctx();
        let list = list.ptr;
