// Automatically generated by isl_bindings_generator.
// LICENSE: MIT

use super::{Context, DimType, Error, Id, LibISLError, Space, Val, ValList};
use libc::uintptr_t;
use std::ffi::{CStr, CString};
use std::os::raw::c_char;

/// Wraps `isl_multi_val`.
pub struct MultiVal {
    pub ptr: uintptr_t,
    pub should_free_on_drop: bool,
}

extern "C" {

    fn isl_multi_val_add(multi1: uintptr_t, multi2: uintptr_t) -> uintptr_t;

    fn isl_multi_val_add_dims(multi: uintptr_t, type_: i32, n: u32) -> uintptr_t;

    fn isl_multi_val_add_val(mv: uintptr_t, v: uintptr_t) -> uintptr_t;

    fn isl_multi_val_align_params(multi: uintptr_t, model: uintptr_t) -> uintptr_t;

    fn isl_multi_val_copy(multi: uintptr_t) -> uintptr_t;

    fn isl_multi_val_dim(multi: uintptr_t, type_: i32) -> i32;

    fn isl_multi_val_drop_dims(multi: uintptr_t, type_: i32, first: u32, n: u32) -> uintptr_t;

    fn isl_multi_val_dump(mv: uintptr_t) -> ();

    fn isl_multi_val_factor_range(multi: uintptr_t) -> uintptr_t;

    fn isl_multi_val_find_dim_by_id(multi: uintptr_t, type_: i32, id: uintptr_t) -> i32;

    fn isl_multi_val_find_dim_by_name(multi: uintptr_t, type_: i32, name: *const c_char) -> i32;

    fn isl_multi_val_flat_range_product(multi1: uintptr_t, multi2: uintptr_t) -> uintptr_t;

    fn isl_multi_val_flatten_range(multi: uintptr_t) -> uintptr_t;

    fn isl_multi_val_free(multi: uintptr_t) -> uintptr_t;

    fn isl_multi_val_from_range(multi: uintptr_t) -> uintptr_t;

    fn isl_multi_val_from_val_list(space: uintptr_t, list: uintptr_t) -> uintptr_t;

    fn isl_multi_val_get_at(multi: uintptr_t, pos: i32) -> uintptr_t;

    fn isl_multi_val_get_ctx(multi: uintptr_t) -> uintptr_t;

    fn isl_multi_val_get_dim_id(multi: uintptr_t, type_: i32, pos: u32) -> uintptr_t;

    fn isl_multi_val_get_domain_space(multi: uintptr_t) -> uintptr_t;

    fn isl_multi_val_get_list(multi: uintptr_t) -> uintptr_t;

    fn isl_multi_val_get_range_tuple_id(multi: uintptr_t) -> uintptr_t;

    fn isl_multi_val_get_space(multi: uintptr_t) -> uintptr_t;

    fn isl_multi_val_get_tuple_id(multi: uintptr_t, type_: i32) -> uintptr_t;

    fn isl_multi_val_get_tuple_name(multi: uintptr_t, type_: i32) -> *const c_char;

    fn isl_multi_val_get_val(multi: uintptr_t, pos: i32) -> uintptr_t;

    fn isl_multi_val_has_range_tuple_id(multi: uintptr_t) -> i32;

    fn isl_multi_val_has_tuple_id(multi: uintptr_t, type_: i32) -> i32;

    fn isl_multi_val_insert_dims(multi: uintptr_t, type_: i32, first: u32, n: u32) -> uintptr_t;

    fn isl_multi_val_involves_dims(multi: uintptr_t, type_: i32, first: u32, n: u32) -> i32;

    fn isl_multi_val_involves_nan(multi: uintptr_t) -> i32;

    fn isl_multi_val_is_zero(mv: uintptr_t) -> i32;

    fn isl_multi_val_max(multi1: uintptr_t, multi2: uintptr_t) -> uintptr_t;

    fn isl_multi_val_min(multi1: uintptr_t, multi2: uintptr_t) -> uintptr_t;

    fn isl_multi_val_mod_multi_val(multi: uintptr_t, mv: uintptr_t) -> uintptr_t;

    fn isl_multi_val_mod_val(mv: uintptr_t, v: uintptr_t) -> uintptr_t;

    fn isl_multi_val_neg(multi: uintptr_t) -> uintptr_t;

    fn isl_multi_val_plain_is_equal(multi1: uintptr_t, multi2: uintptr_t) -> i32;

    fn isl_multi_val_product(multi1: uintptr_t, multi2: uintptr_t) -> uintptr_t;

    fn isl_multi_val_project_domain_on_params(multi: uintptr_t) -> uintptr_t;

    fn isl_multi_val_range_factor_domain(multi: uintptr_t) -> uintptr_t;

    fn isl_multi_val_range_factor_range(multi: uintptr_t) -> uintptr_t;

    fn isl_multi_val_range_is_wrapping(multi: uintptr_t) -> i32;

    fn isl_multi_val_range_product(multi1: uintptr_t, multi2: uintptr_t) -> uintptr_t;

    fn isl_multi_val_range_splice(multi1: uintptr_t, pos: u32, multi2: uintptr_t) -> uintptr_t;

    fn isl_multi_val_read_from_str(ctx: uintptr_t, str_: *const c_char) -> uintptr_t;

    fn isl_multi_val_reset_range_tuple_id(multi: uintptr_t) -> uintptr_t;

    fn isl_multi_val_reset_tuple_id(multi: uintptr_t, type_: i32) -> uintptr_t;

    fn isl_multi_val_reset_user(multi: uintptr_t) -> uintptr_t;

    fn isl_multi_val_scale_down_multi_val(multi: uintptr_t, mv: uintptr_t) -> uintptr_t;

    fn isl_multi_val_scale_down_val(multi: uintptr_t, v: uintptr_t) -> uintptr_t;

    fn isl_multi_val_scale_multi_val(multi: uintptr_t, mv: uintptr_t) -> uintptr_t;

    fn isl_multi_val_scale_val(multi: uintptr_t, v: uintptr_t) -> uintptr_t;

    fn isl_multi_val_set_at(multi: uintptr_t, pos: i32, el: uintptr_t) -> uintptr_t;

    fn isl_multi_val_set_dim_id(multi: uintptr_t, type_: i32, pos: u32, id: uintptr_t)
                                -> uintptr_t;

    fn isl_multi_val_set_dim_name(multi: uintptr_t, type_: i32, pos: u32, s: *const c_char)
                                  -> uintptr_t;

    fn isl_multi_val_set_range_tuple_id(multi: uintptr_t, id: uintptr_t) -> uintptr_t;

    fn isl_multi_val_set_tuple_id(multi: uintptr_t, type_: i32, id: uintptr_t) -> uintptr_t;

    fn isl_multi_val_set_tuple_name(multi: uintptr_t, type_: i32, s: *const c_char) -> uintptr_t;

    fn isl_multi_val_set_val(multi: uintptr_t, pos: i32, el: uintptr_t) -> uintptr_t;

    fn isl_multi_val_size(multi: uintptr_t) -> i32;

    fn isl_multi_val_splice(multi1: uintptr_t, in_pos: u32, out_pos: u32, multi2: uintptr_t)
                            -> uintptr_t;

    fn isl_multi_val_sub(multi1: uintptr_t, multi2: uintptr_t) -> uintptr_t;

    fn isl_multi_val_to_str(mv: uintptr_t) -> *const c_char;

    fn isl_multi_val_zero(space: uintptr_t) -> uintptr_t;

}

impl MultiVal {
    /// Wraps `isl_multi_val_add`.
    pub fn add(self, multi2: MultiVal) -> Result<MultiVal, LibISLError> {
        let multi1 = self;
        let isl_rs_ctx = multi1.get_ctx();
        let mut multi1 = multi1;
        multi1.do_not_free_on_drop();
        let multi1 = multi1.ptr;
        let mut multi2 = multi2;
        multi2.do_not_free_on_drop();
        let multi2 = multi2.ptr;
        let isl_rs_result = unsafe { isl_multi_val_add(multi1, multi2) };
        let isl_rs_result = MultiVal { ptr: isl_rs_result,
                                       should_free_on_drop: true };
        let err = isl_rs_ctx.last_error();
        if err != Error::None_ {
            return Err(LibISLError::new(err, isl_rs_ctx.last_error_msg()));
        }
        Ok(isl_rs_result)
    }

    /// Wraps `isl_multi_val_add_dims`.
    pub fn add_dims(self, type_: DimType, n: u32) -> Result<MultiVal, LibISLError> {
        let multi = self;
        let isl_rs_ctx = multi.get_ctx();
        let mut multi = multi;
        multi.do_not_free_on_drop();
        let multi = multi.ptr;
        let type_ = type_.to_i32();
        let isl_rs_result = unsafe { isl_multi_val_add_dims(multi, type_, n) };
        let isl_rs_result = MultiVal { ptr: isl_rs_result,
                                       should_free_on_drop: true };
        let err = isl_rs_ctx.last_error();
        if err != Error::None_ {
            return Err(LibISLError::new(err, isl_rs_ctx.last_error_msg()));
        }
        Ok(isl_rs_result)
    }

    /// Wraps `isl_multi_val_add_val`.
    pub fn add_val(self, v: Val) -> Result<MultiVal, LibISLError> {
        let mv = self;
        let isl_rs_ctx = mv.get_ctx();
        let mut mv = mv;
        mv.do_not_free_on_drop();
        let mv = mv.ptr;
        let mut v = v;
        v.do_not_free_on_drop();
        let v = v.ptr;
        let isl_rs_result = unsafe { isl_multi_val_add_val(mv, v) };
        let isl_rs_result = MultiVal { ptr: isl_rs_result,
                                       should_free_on_drop: true };
        let err = isl_rs_ctx.last_error();
        if err != Error::None_ {
            return Err(LibISLError::new(err, isl_rs_ctx.last_error_msg()));
        }
        Ok(isl_rs_result)
    }

    /// Wraps `isl_multi_val_align_params`.
    pub fn align_params(self, model: Space) -> Result<MultiVal, LibISLError> {
        let multi = self;
        let isl_rs_ctx = multi.get_ctx();
        let mut multi = multi;
        multi.do_not_free_on_drop();
        let multi = multi.ptr;
        let mut model = model;
        model.do_not_free_on_drop();
        let model = model.ptr;
        let isl_rs_result = unsafe { isl_multi_val_align_params(multi, model) };
        let isl_rs_result = MultiVal { ptr: isl_rs_result,
                                       should_free_on_drop: true };
        let err = isl_rs_ctx.last_error();
        if err != Error::None_ {
            return Err(LibISLError::new(err, isl_rs_ctx.last_error_msg()));
        }
        Ok(isl_rs_result)
    }

    /// Wraps `isl_multi_val_copy`.
    pub fn copy(&self) -> Result<MultiVal, LibISLError> {
        let multi = self;
        let isl_rs_ctx = multi.get_ctx();
        let multi = multi.ptr;
        let isl_rs_result = unsafe { isl_multi_val_copy(multi) };
        let isl_rs_result = MultiVal { ptr: isl_rs_result,
                                       should_free_on_drop: true };
        let err = isl_rs_ctx.last_error();
        if err != Error::None_ {
            return Err(LibISLError::new(err, isl_rs_ctx.last_error_msg()));
        }
        Ok(isl_rs_result)
    }

    /// Wraps `isl_multi_val_dim`.
    pub fn dim(&self, type_: DimType) -> Result<i32, LibISLError> {
        let multi = self;
        let isl_rs_ctx = multi.get_ctx();
        let multi = multi.ptr;
        let type_ = type_.to_i32();
        let isl_rs_result = unsafe { isl_multi_val_dim(multi, type_) };
        let err = isl_rs_ctx.last_error();
        if err != Error::None_ {
            return Err(LibISLError::new(err, isl_rs_ctx.last_error_msg()));
        }
        Ok(isl_rs_result)
    }

    /// Wraps `isl_multi_val_drop_dims`.
    pub fn drop_dims(self, type_: DimType, first: u32, n: u32) -> Result<MultiVal, LibISLError> {
        let multi = self;
        let isl_rs_ctx = multi.get_ctx();
        let mut multi = multi;
        multi.do_not_free_on_drop();
        let multi = multi.ptr;
        let type_ = type_.to_i32();
        let isl_rs_result = unsafe { isl_multi_val_drop_dims(multi, type_, first, n) };
        let isl_rs_result = MultiVal { ptr: isl_rs_result,
                                       should_free_on_drop: true };
        let err = isl_rs_ctx.last_error();
        if err != Error::None_ {
            return Err(LibISLError::new(err, isl_rs_ctx.last_error_msg()));
        }
        Ok(isl_rs_result)
    }

    /// Wraps `isl_multi_val_dump`.
    pub fn dump(&self) -> Result<(), LibISLError> {
        let mv = self;
        let isl_rs_ctx = mv.get_ctx();
        let mv = mv.ptr;
        let isl_rs_result = unsafe { isl_multi_val_dump(mv) };
        let err = isl_rs_ctx.last_error();
        if err != Error::None_ {
            return Err(LibISLError::new(err, isl_rs_ctx.last_error_msg()));
        }
        Ok(isl_rs_result)
    }

    /// Wraps `isl_multi_val_factor_range`.
    pub fn factor_range(self) -> Result<MultiVal, LibISLError> {
        let multi = self;
        let isl_rs_ctx = multi.get_ctx();
        let mut multi = multi;
        multi.do_not_free_on_drop();
        let multi = multi.ptr;
        let isl_rs_result = unsafe { isl_multi_val_factor_range(multi) };
        let isl_rs_result = MultiVal { ptr: isl_rs_result,
                                       should_free_on_drop: true };
        let err = isl_rs_ctx.last_error();
        if err != Error::None_ {
            return Err(LibISLError::new(err, isl_rs_ctx.last_error_msg()));
        }
        Ok(isl_rs_result)
    }

    /// Wraps `isl_multi_val_find_dim_by_id`.
    pub fn find_dim_by_id(&self, type_: DimType, id: &Id) -> Result<i32, LibISLError> {
        let multi = self;
        let isl_rs_ctx = multi.get_ctx();
        let multi = multi.ptr;
        let type_ = type_.to_i32();
        let id = id.ptr;
        let isl_rs_result = unsafe { isl_multi_val_find_dim_by_id(multi, type_, id) };
        let err = isl_rs_ctx.last_error();
        if err != Error::None_ {
            return Err(LibISLError::new(err, isl_rs_ctx.last_error_msg()));
        }
        Ok(isl_rs_result)
    }

    /// Wraps `isl_multi_val_find_dim_by_name`.
    pub fn find_dim_by_name(&self, type_: DimType, name: &str) -> Result<i32, LibISLError> {
        let multi = self;
        let isl_rs_ctx = multi.get_ctx();
        let multi = multi.ptr;
        let type_ = type_.to_i32();
        let name = CString::new(name).unwrap();
        let name = name.as_ptr();
        let isl_rs_result = unsafe { isl_multi_val_find_dim_by_name(multi, type_, name) };
        let err = isl_rs_ctx.last_error();
        if err != Error::None_ {
            return Err(LibISLError::new(err, isl_rs_ctx.last_error_msg()));
        }
        Ok(isl_rs_result)
    }

    /// Wraps `isl_multi_val_flat_range_product`.
    pub fn flat_range_product(self, multi2: MultiVal) -> Result<MultiVal, LibISLError> {
        let multi1 = self;
        let isl_rs_ctx = multi1.get_ctx();
        let mut multi1 = multi1;
        multi1.do_not_free_on_drop();
        let multi1 = multi1.ptr;
        let mut multi2 = multi2;
        multi2.do_not_free_on_drop();
        let multi2 = multi2.ptr;
        let isl_rs_result = unsafe { isl_multi_val_flat_range_product(multi1, multi2) };
        let isl_rs_result = MultiVal { ptr: isl_rs_result,
                                       should_free_on_drop: true };
        let err = isl_rs_ctx.last_error();
        if err != Error::None_ {
            return Err(LibISLError::new(err, isl_rs_ctx.last_error_msg()));
        }
        Ok(isl_rs_result)
    }

    /// Wraps `isl_multi_val_flatten_range`.
    pub fn flatten_range(self) -> Result<MultiVal, LibISLError> {
        let multi = self;
        let isl_rs_ctx = multi.get_ctx();
        let mut multi = multi;
        multi.do_not_free_on_drop();
        let multi = multi.ptr;
        let isl_rs_result = unsafe { isl_multi_val_flatten_range(multi) };
        let isl_rs_result = MultiVal { ptr: isl_rs_result,
                                       should_free_on_drop: true };
        let err = isl_rs_ctx.last_error();
        if err != Error::None_ {
            return Err(LibISLError::new(err, isl_rs_ctx.last_error_msg()));
        }
        Ok(isl_rs_result)
    }

    /// Wraps `isl_multi_val_free`.
    pub fn free(self) -> Result<MultiVal, LibISLError> {
        let multi = self;
        let isl_rs_ctx = multi.get_ctx();
        let mut multi = multi;
        multi.do_not_free_on_drop();
        let multi = multi.ptr;
        let isl_rs_result = unsafe { isl_multi_val_free(multi) };
        let isl_rs_result = MultiVal { ptr: isl_rs_result,
                                       should_free_on_drop: true };
        let err = isl_rs_ctx.last_error();
        if err != Error::None_ {
            return Err(LibISLError::new(err, isl_rs_ctx.last_error_msg()));
        }
        Ok(isl_rs_result)
    }

    /// Wraps `isl_multi_val_from_range`.
    pub fn from_range(self) -> Result<MultiVal, LibISLError> {
        let multi = self;
        let isl_rs_ctx = multi.get_ctx();
        let mut multi = multi;
        multi.do_not_free_on_drop();
        let multi = multi.ptr;
        let isl_rs_result = unsafe { isl_multi_val_from_range(multi) };
        let isl_rs_result = MultiVal { ptr: isl_rs_result,
                                       should_free_on_drop: true };
        let err = isl_rs_ctx.last_error();
        if err != Error::None_ {
            return Err(LibISLError::new(err, isl_rs_ctx.last_error_msg()));
        }
        Ok(isl_rs_result)
    }

    /// Wraps `isl_multi_val_from_val_list`.
    pub fn from_val_list(space: Space, list: ValList) -> Result<MultiVal, LibISLError> {
        let isl_rs_ctx = space.get_ctx();
        let mut space = space;
        space.do_not_free_on_drop();
        let space = space.ptr;
        let mut list = list;
        list.do_not_free_on_drop();
        let list = list.ptr;
        let isl_rs_result = unsafe { isl_multi_val_from_val_list(space, list) };
        let isl_rs_result = MultiVal { ptr: isl_rs_result,
                                       should_free_on_drop: true };
        let err = isl_rs_ctx.last_error();
        if err != Error::None_ {
            return Err(LibISLError::new(err, isl_rs_ctx.last_error_msg()));
        }
        Ok(isl_rs_result)
    }

    /// Wraps `isl_multi_val_get_at`.
    pub fn get_at(&self, pos: i32) -> Result<Val, LibISLError> {
        let multi = self;
        let isl_rs_ctx = multi.get_ctx();
        let multi = multi.ptr;
        let isl_rs_result = unsafe { isl_multi_val_get_at(multi, pos) };
        let isl_rs_result = Val { ptr: isl_rs_result,
                                  should_free_on_drop: true };
        let err = isl_rs_ctx.last_error();
        if err != Error::None_ {
            return Err(LibISLError::new(err, isl_rs_ctx.last_error_msg()));
        }
        Ok(isl_rs_result)
    }

    /// Wraps `isl_multi_val_get_ctx`.
    pub fn get_ctx(&self) -> Context {
        let multi = self;
        let multi = multi.ptr;
        let isl_rs_result = unsafe { isl_multi_val_get_ctx(multi) };
        let isl_rs_result = Context { ptr: isl_rs_result,
                                      should_free_on_drop: false };
        isl_rs_result
    }

    /// Wraps `isl_multi_val_get_dim_id`.
    pub fn get_dim_id(&self, type_: DimType, pos: u32) -> Result<Id, LibISLError> {
        let multi = self;
        let isl_rs_ctx = multi.get_ctx();
        let multi = multi.ptr;
        let type_ = type_.to_i32();
        let isl_rs_result = unsafe { isl_multi_val_get_dim_id(multi, type_, pos) };
        let isl_rs_result = Id { ptr: isl_rs_result,
                                 should_free_on_drop: true };
        let err = isl_rs_ctx.last_error();
        if err != Error::None_ {
            return Err(LibISLError::new(err, isl_rs_ctx.last_error_msg()));
        }
        Ok(isl_rs_result)
    }

    /// Wraps `isl_multi_val_get_domain_space`.
    pub fn get_domain_space(&self) -> Result<Space, LibISLError> {
        let multi = self;
        let isl_rs_ctx = multi.get_ctx();
        let multi = multi.ptr;
        let isl_rs_result = unsafe { isl_multi_val_get_domain_space(multi) };
        let isl_rs_result = Space { ptr: isl_rs_result,
                                    should_free_on_drop: true };
        let err = isl_rs_ctx.last_error();
        if err != Error::None_ {
            return Err(LibISLError::new(err, isl_rs_ctx.last_error_msg()));
        }
        Ok(isl_rs_result)
    }

    /// Wraps `isl_multi_val_get_list`.
    pub fn get_list(&self) -> Result<ValList, LibISLError> {
        let multi = self;
        let isl_rs_ctx = multi.get_ctx();
        let multi = multi.ptr;
        let isl_rs_result = unsafe { isl_multi_val_get_list(multi) };
        let isl_rs_result = ValList { ptr: isl_rs_result,
                                      should_free_on_drop: true };
        let err = isl_rs_ctx.last_error();
        if err != Error::None_ {
            return Err(LibISLError::new(err, isl_rs_ctx.last_error_msg()));
        }
        Ok(isl_rs_result)
    }

    /// Wraps `isl_multi_val_get_range_tuple_id`.
    pub fn get_range_tuple_id(&self) -> Result<Id, LibISLError> {
        let multi = self;
        let isl_rs_ctx = multi.get_ctx();
        let multi = multi.ptr;
        let isl_rs_result = unsafe { isl_multi_val_get_range_tuple_id(multi) };
        let isl_rs_result = Id { ptr: isl_rs_result,
                                 should_free_on_drop: true };
        let err = isl_rs_ctx.last_error();
        if err != Error::None_ {
            return Err(LibISLError::new(err, isl_rs_ctx.last_error_msg()));
        }
        Ok(isl_rs_result)
    }

    /// Wraps `isl_multi_val_get_space`.
    pub fn get_space(&self) -> Result<Space, LibISLError> {
        let multi = self;
        let isl_rs_ctx = multi.get_ctx();
        let multi = multi.ptr;
        let isl_rs_result = unsafe { isl_multi_val_get_space(multi) };
        let isl_rs_result = Space { ptr: isl_rs_result,
                                    should_free_on_drop: true };
        let err = isl_rs_ctx.last_error();
        if err != Error::None_ {
            return Err(LibISLError::new(err, isl_rs_ctx.last_error_msg()));
        }
        Ok(isl_rs_result)
    }

    /// Wraps `isl_multi_val_get_tuple_id`.
    pub fn get_tuple_id(&self, type_: DimType) -> Result<Id, LibISLError> {
        let multi = self;
        let isl_rs_ctx = multi.get_ctx();
        let multi = multi.ptr;
        let type_ = type_.to_i32();
        let isl_rs_result = unsafe { isl_multi_val_get_tuple_id(multi, type_) };
        let isl_rs_result = Id { ptr: isl_rs_result,
                                 should_free_on_drop: true };
        let err = isl_rs_ctx.last_error();
        if err != Error::None_ {
            return Err(LibISLError::new(err, isl_rs_ctx.last_error_msg()));
        }
        Ok(isl_rs_result)
    }

    /// Wraps `isl_multi_val_get_tuple_name`.
    pub fn get_tuple_name(&self, type_: DimType) -> Result<&str, LibISLError> {
        let multi = self;
        let isl_rs_ctx = multi.get_ctx();
        let multi = multi.ptr;
        let type_ = type_.to_i32();
        let isl_rs_result = unsafe { isl_multi_val_get_tuple_name(multi, type_) };
        let isl_rs_result = unsafe { CStr::from_ptr(isl_rs_result) };
        let isl_rs_result = isl_rs_result.to_str().unwrap();
        let err = isl_rs_ctx.last_error();
        if err != Error::None_ {
            return Err(LibISLError::new(err, isl_rs_ctx.last_error_msg()));
        }
        Ok(isl_rs_result)
    }

    /// Wraps `isl_multi_val_get_val`.
    pub fn get_val(&self, pos: i32) -> Result<Val, LibISLError> {
        let multi = self;
        let isl_rs_ctx = multi.get_ctx();
        let multi = multi.ptr;
        let isl_rs_result = unsafe { isl_multi_val_get_val(multi, pos) };
        let isl_rs_result = Val { ptr: isl_rs_result,
                                  should_free_on_drop: true };
        let err = isl_rs_ctx.last_error();
        if err != Error::None_ {
            return Err(LibISLError::new(err, isl_rs_ctx.last_error_msg()));
        }
        Ok(isl_rs_result)
    }

    /// Wraps `isl_multi_val_has_range_tuple_id`.
    pub fn has_range_tuple_id(&self) -> Result<bool, LibISLError> {
        let multi = self;
        let isl_rs_ctx = multi.get_ctx();
        let multi = multi.ptr;
        let isl_rs_result = unsafe { isl_multi_val_has_range_tuple_id(multi) };
        let isl_rs_result = match isl_rs_result {
            0 => false,
            1 => true,
            _ => panic!("Got isl_bool = -1"),
        };
        let err = isl_rs_ctx.last_error();
        if err != Error::None_ {
            return Err(LibISLError::new(err, isl_rs_ctx.last_error_msg()));
        }
        Ok(isl_rs_result)
    }

    /// Wraps `isl_multi_val_has_tuple_id`.
    pub fn has_tuple_id(&self, type_: DimType) -> Result<bool, LibISLError> {
        let multi = self;
        let isl_rs_ctx = multi.get_ctx();
        let multi = multi.ptr;
        let type_ = type_.to_i32();
        let isl_rs_result = unsafe { isl_multi_val_has_tuple_id(multi, type_) };
        let isl_rs_result = match isl_rs_result {
            0 => false,
            1 => true,
            _ => panic!("Got isl_bool = -1"),
        };
        let err = isl_rs_ctx.last_error();
        if err != Error::None_ {
            return Err(LibISLError::new(err, isl_rs_ctx.last_error_msg()));
        }
        Ok(isl_rs_result)
    }

    /// Wraps `isl_multi_val_insert_dims`.
    pub fn insert_dims(self, type_: DimType, first: u32, n: u32) -> Result<MultiVal, LibISLError> {
        let multi = self;
        let isl_rs_ctx = multi.get_ctx();
        let mut multi = multi;
        multi.do_not_free_on_drop();
        let multi = multi.ptr;
        let type_ = type_.to_i32();
        let isl_rs_result = unsafe { isl_multi_val_insert_dims(multi, type_, first, n) };
        let isl_rs_result = MultiVal { ptr: isl_rs_result,
                                       should_free_on_drop: true };
        let err = isl_rs_ctx.last_error();
        if err != Error::None_ {
            return Err(LibISLError::new(err, isl_rs_ctx.last_error_msg()));
        }
        Ok(isl_rs_result)
    }

    /// Wraps `isl_multi_val_involves_dims`.
    pub fn involves_dims(&self, type_: DimType, first: u32, n: u32) -> Result<bool, LibISLError> {
        let multi = self;
        let isl_rs_ctx = multi.get_ctx();
        let multi = multi.ptr;
        let type_ = type_.to_i32();
        let isl_rs_result = unsafe { isl_multi_val_involves_dims(multi, type_, first, n) };
        let isl_rs_result = match isl_rs_result {
            0 => false,
            1 => true,
            _ => panic!("Got isl_bool = -1"),
        };
        let err = isl_rs_ctx.last_error();
        if err != Error::None_ {
            return Err(LibISLError::new(err, isl_rs_ctx.last_error_msg()));
        }
        Ok(isl_rs_result)
    }

    /// Wraps `isl_multi_val_involves_nan`.
    pub fn involves_nan(&self) -> Result<bool, LibISLError> {
        let multi = self;
        let isl_rs_ctx = multi.get_ctx();
        let multi = multi.ptr;
        let isl_rs_result = unsafe { isl_multi_val_involves_nan(multi) };
        let isl_rs_result = match isl_rs_result {
            0 => false,
            1 => true,
            _ => panic!("Got isl_bool = -1"),
        };
        let err = isl_rs_ctx.last_error();
        if err != Error::None_ {
            return Err(LibISLError::new(err, isl_rs_ctx.last_error_msg()));
        }
        Ok(isl_rs_result)
    }

    /// Wraps `isl_multi_val_is_zero`.
    pub fn is_zero(&self) -> Result<bool, LibISLError> {
        let mv = self;
        let isl_rs_ctx = mv.get_ctx();
        let mv = mv.ptr;
        let isl_rs_result = unsafe { isl_multi_val_is_zero(mv) };
        let isl_rs_result = match isl_rs_result {
            0 => false,
            1 => true,
            _ => panic!("Got isl_bool = -1"),
        };
        let err = isl_rs_ctx.last_error();
        if err != Error::None_ {
            return Err(LibISLError::new(err, isl_rs_ctx.last_error_msg()));
        }
        Ok(isl_rs_result)
    }

    /// Wraps `isl_multi_val_max`.
    pub fn max(self, multi2: MultiVal) -> Result<MultiVal, LibISLError> {
        let multi1 = self;
        let isl_rs_ctx = multi1.get_ctx();
        let mut multi1 = multi1;
        multi1.do_not_free_on_drop();
        let multi1 = multi1.ptr;
        let mut multi2 = multi2;
        multi2.do_not_free_on_drop();
        let multi2 = multi2.ptr;
        let isl_rs_result = unsafe { isl_multi_val_max(multi1, multi2) };
        let isl_rs_result = MultiVal { ptr: isl_rs_result,
                                       should_free_on_drop: true };
        let err = isl_rs_ctx.last_error();
        if err != Error::None_ {
            return Err(LibISLError::new(err, isl_rs_ctx.last_error_msg()));
        }
        Ok(isl_rs_result)
    }

    /// Wraps `isl_multi_val_min`.
    pub fn min(self, multi2: MultiVal) -> Result<MultiVal, LibISLError> {
        let multi1 = self;
        let isl_rs_ctx = multi1.get_ctx();
        let mut multi1 = multi1;
        multi1.do_not_free_on_drop();
        let multi1 = multi1.ptr;
        let mut multi2 = multi2;
        multi2.do_not_free_on_drop();
        let multi2 = multi2.ptr;
        let isl_rs_result = unsafe { isl_multi_val_min(multi1, multi2) };
        let isl_rs_result = MultiVal { ptr: isl_rs_result,
                                       should_free_on_drop: true };
        let err = isl_rs_ctx.last_error();
        if err != Error::None_ {
            return Err(LibISLError::new(err, isl_rs_ctx.last_error_msg()));
        }
        Ok(isl_rs_result)
    }

    /// Wraps `isl_multi_val_mod_multi_val`.
    pub fn mod_multi_val(self, mv: MultiVal) -> Result<MultiVal, LibISLError> {
        let multi = self;
        let isl_rs_ctx = multi.get_ctx();
        let mut multi = multi;
        multi.do_not_free_on_drop();
        let multi = multi.ptr;
        let mut mv = mv;
        mv.do_not_free_on_drop();
        let mv = mv.ptr;
        let isl_rs_result = unsafe { isl_multi_val_mod_multi_val(multi, mv) };
        let isl_rs_result = MultiVal { ptr: isl_rs_result,
                                       should_free_on_drop: true };
        let err = isl_rs_ctx.last_error();
        if err != Error::None_ {
            return Err(LibISLError::new(err, isl_rs_ctx.last_error_msg()));
        }
        Ok(isl_rs_result)
    }

    /// Wraps `isl_multi_val_mod_val`.
    pub fn mod_val(self, v: Val) -> Result<MultiVal, LibISLError> {
        let mv = self;
        let isl_rs_ctx = mv.get_ctx();
        let mut mv = mv;
        mv.do_not_free_on_drop();
        let mv = mv.ptr;
        let mut v = v;
        v.do_not_free_on_drop();
        let v = v.ptr;
        let isl_rs_result = unsafe { isl_multi_val_mod_val(mv, v) };
        let isl_rs_result = MultiVal { ptr: isl_rs_result,
                                       should_free_on_drop: true };
        let err = isl_rs_ctx.last_error();
        if err != Error::None_ {
            return Err(LibISLError::new(err, isl_rs_ctx.last_error_msg()));
        }
        Ok(isl_rs_result)
    }

    /// Wraps `isl_multi_val_neg`.
    pub fn neg(self) -> Result<MultiVal, LibISLError> {
        let multi = self;
        let isl_rs_ctx = multi.get_ctx();
        let mut multi = multi;
        multi.do_not_free_on_drop();
        let multi = multi.ptr;
        let isl_rs_result = unsafe { isl_multi_val_neg(multi) };
        let isl_rs_result = MultiVal { ptr: isl_rs_result,
                                       should_free_on_drop: true };
        let err = isl_rs_ctx.last_error();
        if err != Error::None_ {
            return Err(LibISLError::new(err, isl_rs_ctx.last_error_msg()));
        }
        Ok(isl_rs_result)
    }

    /// Wraps `isl_multi_val_plain_is_equal`.
    pub fn plain_is_equal(&self, multi2: &MultiVal) -> Result<bool, LibISLError> {
        let multi1 = self;
        let isl_rs_ctx = multi1.get_ctx();
        let multi1 = multi1.ptr;
        let multi2 = multi2.ptr;
        let isl_rs_result = unsafe { isl_multi_val_plain_is_equal(multi1, multi2) };
        let isl_rs_result = match isl_rs_result {
            0 => false,
            1 => true,
            _ => panic!("Got isl_bool = -1"),
        };
        let err = isl_rs_ctx.last_error();
        if err != Error::None_ {
            return Err(LibISLError::new(err, isl_rs_ctx.last_error_msg()));
        }
        Ok(isl_rs_result)
    }

    /// Wraps `isl_multi_val_product`.
    pub fn product(self, multi2: MultiVal) -> Result<MultiVal, LibISLError> {
        let multi1 = self;
        let isl_rs_ctx = multi1.get_ctx();
        let mut multi1 = multi1;
        multi1.do_not_free_on_drop();
        let multi1 = multi1.ptr;
        let mut multi2 = multi2;
        multi2.do_not_free_on_drop();
        let multi2 = multi2.ptr;
        let isl_rs_result = unsafe { isl_multi_val_product(multi1, multi2) };
        let isl_rs_result = MultiVal { ptr: isl_rs_result,
                                       should_free_on_drop: true };
        let err = isl_rs_ctx.last_error();
        if err != Error::None_ {
            return Err(LibISLError::new(err, isl_rs_ctx.last_error_msg()));
        }
        Ok(isl_rs_result)
    }

    /// Wraps `isl_multi_val_project_domain_on_params`.
    pub fn project_domain_on_params(self) -> Result<MultiVal, LibISLError> {
        let multi = self;
        let isl_rs_ctx = multi.get_ctx();
        let mut multi = multi;
        multi.do_not_free_on_drop();
        let multi = multi.ptr;
        let isl_rs_result = unsafe { isl_multi_val_project_domain_on_params(multi) };
        let isl_rs_result = MultiVal { ptr: isl_rs_result,
                                       should_free_on_drop: true };
        let err = isl_rs_ctx.last_error();
        if err != Error::None_ {
            return Err(LibISLError::new(err, isl_rs_ctx.last_error_msg()));
        }
        Ok(isl_rs_result)
    }

    /// Wraps `isl_multi_val_range_factor_domain`.
    pub fn range_factor_domain(self) -> Result<MultiVal, LibISLError> {
        let multi = self;
        let isl_rs_ctx = multi.get_ctx();
        let mut multi = multi;
        multi.do_not_free_on_drop();
        let multi = multi.ptr;
        let isl_rs_result = unsafe { isl_multi_val_range_factor_domain(multi) };
        let isl_rs_result = MultiVal { ptr: isl_rs_result,
                                       should_free_on_drop: true };
        let err = isl_rs_ctx.last_error();
        if err != Error::None_ {
            return Err(LibISLError::new(err, isl_rs_ctx.last_error_msg()));
        }
        Ok(isl_rs_result)
    }

    /// Wraps `isl_multi_val_range_factor_range`.
    pub fn range_factor_range(self) -> Result<MultiVal, LibISLError> {
        let multi = self;
        let isl_rs_ctx = multi.get_ctx();
        let mut multi = multi;
        multi.do_not_free_on_drop();
        let multi = multi.ptr;
        let isl_rs_result = unsafe { isl_multi_val_range_factor_range(multi) };
        let isl_rs_result = MultiVal { ptr: isl_rs_result,
                                       should_free_on_drop: true };
        let err = isl_rs_ctx.last_error();
        if err != Error::None_ {
            return Err(LibISLError::new(err, isl_rs_ctx.last_error_msg()));
        }
        Ok(isl_rs_result)
    }

    /// Wraps `isl_multi_val_range_is_wrapping`.
    pub fn range_is_wrapping(&self) -> Result<bool, LibISLError> {
        let multi = self;
        let isl_rs_ctx = multi.get_ctx();
        let multi = multi.ptr;
        let isl_rs_result = unsafe { isl_multi_val_range_is_wrapping(multi) };
        let isl_rs_result = match isl_rs_result {
            0 => false,
            1 => true,
            _ => panic!("Got isl_bool = -1"),
        };
        let err = isl_rs_ctx.last_error();
        if err != Error::None_ {
            return Err(LibISLError::new(err, isl_rs_ctx.last_error_msg()));
        }
        Ok(isl_rs_result)
    }

    /// Wraps `isl_multi_val_range_product`.
    pub fn range_product(self, multi2: MultiVal) -> Result<MultiVal, LibISLError> {
        let multi1 = self;
        let isl_rs_ctx = multi1.get_ctx();
        let mut multi1 = multi1;
        multi1.do_not_free_on_drop();
        let multi1 = multi1.ptr;
        let mut multi2 = multi2;
        multi2.do_not_free_on_drop();
        let multi2 = multi2.ptr;
        let isl_rs_result = unsafe { isl_multi_val_range_product(multi1, multi2) };
        let isl_rs_result = MultiVal { ptr: isl_rs_result,
                                       should_free_on_drop: true };
        let err = isl_rs_ctx.last_error();
        if err != Error::None_ {
            return Err(LibISLError::new(err, isl_rs_ctx.last_error_msg()));
        }
        Ok(isl_rs_result)
    }

    /// Wraps `isl_multi_val_range_splice`.
    pub fn range_splice(self, pos: u32, multi2: MultiVal) -> Result<MultiVal, LibISLError> {
        let multi1 = self;
        let isl_rs_ctx = multi1.get_ctx();
        let mut multi1 = multi1;
        multi1.do_not_free_on_drop();
        let multi1 = multi1.ptr;
        let mut multi2 = multi2;
        multi2.do_not_free_on_drop();
        let multi2 = multi2.ptr;
        let isl_rs_result = unsafe { isl_multi_val_range_splice(multi1, pos, multi2) };
        let isl_rs_result = MultiVal { ptr: isl_rs_result,
                                       should_free_on_drop: true };
        let err = isl_rs_ctx.last_error();
        if err != Error::None_ {
            return Err(LibISLError::new(err, isl_rs_ctx.last_error_msg()));
        }
        Ok(isl_rs_result)
    }

    /// Wraps `isl_multi_val_read_from_str`.
    pub fn read_from_str(ctx: &Context, str_: &str) -> Result<MultiVal, LibISLError> {
        let isl_rs_ctx = Context { ptr: ctx.ptr,
                                   should_free_on_drop: false };
        let ctx = ctx.ptr;
        let str_ = CString::new(str_).unwrap();
        let str_ = str_.as_ptr();
        let isl_rs_result = unsafe { isl_multi_val_read_from_str(ctx, str_) };
        let isl_rs_result = MultiVal { ptr: isl_rs_result,
                                       should_free_on_drop: true };
        let err = isl_rs_ctx.last_error();
        if err != Error::None_ {
            return Err(LibISLError::new(err, isl_rs_ctx.last_error_msg()));
        }
        Ok(isl_rs_result)
    }

    /// Wraps `isl_multi_val_reset_range_tuple_id`.
    pub fn reset_range_tuple_id(self) -> Result<MultiVal, LibISLError> {
        let multi = self;
        let isl_rs_ctx = multi.get_ctx();
        let mut multi = multi;
        multi.do_not_free_on_drop();
        let multi = multi.ptr;
        let isl_rs_result = unsafe { isl_multi_val_reset_range_tuple_id(multi) };
        let isl_rs_result = MultiVal { ptr: isl_rs_result,
                                       should_free_on_drop: true };
        let err = isl_rs_ctx.last_error();
        if err != Error::None_ {
            return Err(LibISLError::new(err, isl_rs_ctx.last_error_msg()));
        }
        Ok(isl_rs_result)
    }

    /// Wraps `isl_multi_val_reset_tuple_id`.
    pub fn reset_tuple_id(self, type_: DimType) -> Result<MultiVal, LibISLError> {
        let multi = self;
        let isl_rs_ctx = multi.get_ctx();
        let mut multi = multi;
        multi.do_not_free_on_drop();
        let multi = multi.ptr;
        let type_ = type_.to_i32();
        let isl_rs_result = unsafe { isl_multi_val_reset_tuple_id(multi, type_) };
        let isl_rs_result = MultiVal { ptr: isl_rs_result,
                                       should_free_on_drop: true };
        let err = isl_rs_ctx.last_error();
        if err != Error::None_ {
            return Err(LibISLError::new(err, isl_rs_ctx.last_error_msg()));
        }
        Ok(isl_rs_result)
    }

    /// Wraps `isl_multi_val_reset_user`.
    pub fn reset_user(self) -> Result<MultiVal, LibISLError> {
        let multi = self;
        let isl_rs_ctx = multi.get_ctx();
        let mut multi = multi;
        multi.do_not_free_on_drop();
        let multi = multi.ptr;
        let isl_rs_result = unsafe { isl_multi_val_reset_user(multi) };
        let isl_rs_result = MultiVal { ptr: isl_rs_result,
                                       should_free_on_drop: true };
        let err = isl_rs_ctx.last_error();
        if err != Error::None_ {
            return Err(LibISLError::new(err, isl_rs_ctx.last_error_msg()));
        }
        Ok(isl_rs_result)
    }

    /// Wraps `isl_multi_val_scale_down_multi_val`.
    pub fn scale_down_multi_val(self, mv: MultiVal) -> Result<MultiVal, LibISLError> {
        let multi = self;
        let isl_rs_ctx = multi.get_ctx();
        let mut multi = multi;
        multi.do_not_free_on_drop();
        let multi = multi.ptr;
        let mut mv = mv;
        mv.do_not_free_on_drop();
        let mv = mv.ptr;
        let isl_rs_result = unsafe { isl_multi_val_scale_down_multi_val(multi, mv) };
        let isl_rs_result = MultiVal { ptr: isl_rs_result,
                                       should_free_on_drop: true };
        let err = isl_rs_ctx.last_error();
        if err != Error::None_ {
            return Err(LibISLError::new(err, isl_rs_ctx.last_error_msg()));
        }
        Ok(isl_rs_result)
    }

    /// Wraps `isl_multi_val_scale_down_val`.
    pub fn scale_down_val(self, v: Val) -> Result<MultiVal, LibISLError> {
        let multi = self;
        let isl_rs_ctx = multi.get_ctx();
        let mut multi = multi;
        multi.do_not_free_on_drop();
        let multi = multi.ptr;
        let mut v = v;
        v.do_not_free_on_drop();
        let v = v.ptr;
        let isl_rs_result = unsafe { isl_multi_val_scale_down_val(multi, v) };
        let isl_rs_result = MultiVal { ptr: isl_rs_result,
                                       should_free_on_drop: true };
        let err = isl_rs_ctx.last_error();
        if err != Error::None_ {
            return Err(LibISLError::new(err, isl_rs_ctx.last_error_msg()));
        }
        Ok(isl_rs_result)
    }

    /// Wraps `isl_multi_val_scale_multi_val`.
    pub fn scale_multi_val(self, mv: MultiVal) -> Result<MultiVal, LibISLError> {
        let multi = self;
        let isl_rs_ctx = multi.get_ctx();
        let mut multi = multi;
        multi.do_not_free_on_drop();
        let multi = multi.ptr;
        let mut mv = mv;
        mv.do_not_free_on_drop();
        let mv = mv.ptr;
        let isl_rs_result = unsafe { isl_multi_val_scale_multi_val(multi, mv) };
        let isl_rs_result = MultiVal { ptr: isl_rs_result,
                                       should_free_on_drop: true };
        let err = isl_rs_ctx.last_error();
        if err != Error::None_ {
            return Err(LibISLError::new(err, isl_rs_ctx.last_error_msg()));
        }
        Ok(isl_rs_result)
    }

    /// Wraps `isl_multi_val_scale_val`.
    pub fn scale_val(self, v: Val) -> Result<MultiVal, LibISLError> {
        let multi = self;
        let isl_rs_ctx = multi.get_ctx();
        let mut multi = multi;
        multi.do_not_free_on_drop();
        let multi = multi.ptr;
        let mut v = v;
        v.do_not_free_on_drop();
        let v = v.ptr;
        let isl_rs_result = unsafe { isl_multi_val_scale_val(multi, v) };
        let isl_rs_result = MultiVal { ptr: isl_rs_result,
                                       should_free_on_drop: true };
        let err = isl_rs_ctx.last_error();
        if err != Error::None_ {
            return Err(LibISLError::new(err, isl_rs_ctx.last_error_msg()));
        }
        Ok(isl_rs_result)
    }

    /// Wraps `isl_multi_val_set_at`.
    pub fn set_at(self, pos: i32, el: Val) -> Result<MultiVal, LibISLError> {
        let multi = self;
        let isl_rs_ctx = multi.get_ctx();
        let mut multi = multi;
        multi.do_not_free_on_drop();
        let multi = multi.ptr;
        let mut el = el;
        el.do_not_free_on_drop();
        let el = el.ptr;
        let isl_rs_result = unsafe { isl_multi_val_set_at(multi, pos, el) };
        let isl_rs_result = MultiVal { ptr: isl_rs_result,
                                       should_free_on_drop: true };
        let err = isl_rs_ctx.last_error();
        if err != Error::None_ {
            return Err(LibISLError::new(err, isl_rs_ctx.last_error_msg()));
        }
        Ok(isl_rs_result)
    }

    /// Wraps `isl_multi_val_set_dim_id`.
    pub fn set_dim_id(self, type_: DimType, pos: u32, id: Id) -> Result<MultiVal, LibISLError> {
        let multi = self;
        let isl_rs_ctx = multi.get_ctx();
        let mut multi = multi;
        multi.do_not_free_on_drop();
        let multi = multi.ptr;
        let type_ = type_.to_i32();
        let mut id = id;
        id.do_not_free_on_drop();
        let id = id.ptr;
        let isl_rs_result = unsafe { isl_multi_val_set_dim_id(multi, type_, pos, id) };
        let isl_rs_result = MultiVal { ptr: isl_rs_result,
                                       should_free_on_drop: true };
        let err = isl_rs_ctx.last_error();
        if err != Error::None_ {
            return Err(LibISLError::new(err, isl_rs_ctx.last_error_msg()));
        }
        Ok(isl_rs_result)
    }

    /// Wraps `isl_multi_val_set_dim_name`.
    pub fn set_dim_name(self, type_: DimType, pos: u32, s: &str) -> Result<MultiVal, LibISLError> {
        let multi = self;
        let isl_rs_ctx = multi.get_ctx();
        let mut multi = multi;
        multi.do_not_free_on_drop();
        let multi = multi.ptr;
        let type_ = type_.to_i32();
        let s = CString::new(s).unwrap();
        let s = s.as_ptr();
        let isl_rs_result = unsafe { isl_multi_val_set_dim_name(multi, type_, pos, s) };
        let isl_rs_result = MultiVal { ptr: isl_rs_result,
                                       should_free_on_drop: true };
        let err = isl_rs_ctx.last_error();
        if err != Error::None_ {
            return Err(LibISLError::new(err, isl_rs_ctx.last_error_msg()));
        }
        Ok(isl_rs_result)
    }

    /// Wraps `isl_multi_val_set_range_tuple_id`.
    pub fn set_range_tuple_id(self, id: Id) -> Result<MultiVal, LibISLError> {
        let multi = self;
        let isl_rs_ctx = multi.get_ctx();
        let mut multi = multi;
        multi.do_not_free_on_drop();
        let multi = multi.ptr;
        let mut id = id;
        id.do_not_free_on_drop();
        let id = id.ptr;
        let isl_rs_result = unsafe { isl_multi_val_set_range_tuple_id(multi, id) };
        let isl_rs_result = MultiVal { ptr: isl_rs_result,
                                       should_free_on_drop: true };
        let err = isl_rs_ctx.last_error();
        if err != Error::None_ {
            return Err(LibISLError::new(err, isl_rs_ctx.last_error_msg()));
        }
        Ok(isl_rs_result)
    }

    /// Wraps `isl_multi_val_set_tuple_id`.
    pub fn set_tuple_id(self, type_: DimType, id: Id) -> Result<MultiVal, LibISLError> {
        let multi = self;
        let isl_rs_ctx = multi.get_ctx();
        let mut multi = multi;
        multi.do_not_free_on_drop();
        let multi = multi.ptr;
        let type_ = type_.to_i32();
        let mut id = id;
        id.do_not_free_on_drop();
        let id = id.ptr;
        let isl_rs_result = unsafe { isl_multi_val_set_tuple_id(multi, type_, id) };
        let isl_rs_result = MultiVal { ptr: isl_rs_result,
                                       should_free_on_drop: true };
        let err = isl_rs_ctx.last_error();
        if err != Error::None_ {
            return Err(LibISLError::new(err, isl_rs_ctx.last_error_msg()));
        }
        Ok(isl_rs_result)
    }

    /// Wraps `isl_multi_val_set_tuple_name`.
    pub fn set_tuple_name(self, type_: DimType, s: &str) -> Result<MultiVal, LibISLError> {
        let multi = self;
        let isl_rs_ctx = multi.get_ctx();
        let mut multi = multi;
        multi.do_not_free_on_drop();
        let multi = multi.ptr;
        let type_ = type_.to_i32();
        let s = CString::new(s).unwrap();
        let s = s.as_ptr();
        let isl_rs_result = unsafe { isl_multi_val_set_tuple_name(multi, type_, s) };
        let isl_rs_result = MultiVal { ptr: isl_rs_result,
                                       should_free_on_drop: true };
        let err = isl_rs_ctx.last_error();
        if err != Error::None_ {
            return Err(LibISLError::new(err, isl_rs_ctx.last_error_msg()));
        }
        Ok(isl_rs_result)
    }

    /// Wraps `isl_multi_val_set_val`.
    pub fn set_val(self, pos: i32, el: Val) -> Result<MultiVal, LibISLError> {
        let multi = self;
        let isl_rs_ctx = multi.get_ctx();
        let mut multi = multi;
        multi.do_not_free_on_drop();
        let multi = multi.ptr;
        let mut el = el;
        el.do_not_free_on_drop();
        let el = el.ptr;
        let isl_rs_result = unsafe { isl_multi_val_set_val(multi, pos, el) };
        let isl_rs_result = MultiVal { ptr: isl_rs_result,
                                       should_free_on_drop: true };
        let err = isl_rs_ctx.last_error();
        if err != Error::None_ {
            return Err(LibISLError::new(err, isl_rs_ctx.last_error_msg()));
        }
        Ok(isl_rs_result)
    }

    /// Wraps `isl_multi_val_size`.
    pub fn size(&self) -> Result<i32, LibISLError> {
        let multi = self;
        let isl_rs_ctx = multi.get_ctx();
        let multi = multi.ptr;
        let isl_rs_result = unsafe { isl_multi_val_size(multi) };
        let err = isl_rs_ctx.last_error();
        if err != Error::None_ {
            return Err(LibISLError::new(err, isl_rs_ctx.last_error_msg()));
        }
        Ok(isl_rs_result)
    }

    /// Wraps `isl_multi_val_splice`.
    pub fn splice(self, in_pos: u32, out_pos: u32, multi2: MultiVal)
                  -> Result<MultiVal, LibISLError> {
        let multi1 = self;
        let isl_rs_ctx = multi1.get_ctx();
        let mut multi1 = multi1;
        multi1.do_not_free_on_drop();
        let multi1 = multi1.ptr;
        let mut multi2 = multi2;
        multi2.do_not_free_on_drop();
        let multi2 = multi2.ptr;
        let isl_rs_result = unsafe { isl_multi_val_splice(multi1, in_pos, out_pos, multi2) };
        let isl_rs_result = MultiVal { ptr: isl_rs_result,
                                       should_free_on_drop: true };
        let err = isl_rs_ctx.last_error();
        if err != Error::None_ {
            return Err(LibISLError::new(err, isl_rs_ctx.last_error_msg()));
        }
        Ok(isl_rs_result)
    }

    /// Wraps `isl_multi_val_sub`.
    pub fn sub(self, multi2: MultiVal) -> Result<MultiVal, LibISLError> {
        let multi1 = self;
        let isl_rs_ctx = multi1.get_ctx();
        let mut multi1 = multi1;
        multi1.do_not_free_on_drop();
        let multi1 = multi1.ptr;
        let mut multi2 = multi2;
        multi2.do_not_free_on_drop();
        let multi2 = multi2.ptr;
        let isl_rs_result = unsafe { isl_multi_val_sub(multi1, multi2) };
        let isl_rs_result = MultiVal { ptr: isl_rs_result,
                                       should_free_on_drop: true };
        let err = isl_rs_ctx.last_error();
        if err != Error::None_ {
            return Err(LibISLError::new(err, isl_rs_ctx.last_error_msg()));
        }
        Ok(isl_rs_result)
    }

    /// Wraps `isl_multi_val_to_str`.
    pub fn to_str(&self) -> Result<&str, LibISLError> {
        let mv = self;
        let isl_rs_ctx = mv.get_ctx();
        let mv = mv.ptr;
        let isl_rs_result = unsafe { isl_multi_val_to_str(mv) };
        let isl_rs_result = unsafe { CStr::from_ptr(isl_rs_result) };
        let isl_rs_result = isl_rs_result.to_str().unwrap();
        let err = isl_rs_ctx.last_error();
        if err != Error::None_ {
            return Err(LibISLError::new(err, isl_rs_ctx.last_error_msg()));
        }
        Ok(isl_rs_result)
    }

    /// Wraps `isl_multi_val_zero`.
    pub fn zero(space: Space) -> Result<MultiVal, LibISLError> {
        let isl_rs_ctx = space.get_ctx();
        let mut space = space;
        space.do_not_free_on_drop();
        let space = space.ptr;
        let isl_rs_result = unsafe { isl_multi_val_zero(space) };
        let isl_rs_result = MultiVal { ptr: isl_rs_result,
                                       should_free_on_drop: true };
        let err = isl_rs_ctx.last_error();
        if err != Error::None_ {
            return Err(LibISLError::new(err, isl_rs_ctx.last_error_msg()));
        }
        Ok(isl_rs_result)
    }

    /// Does not call isl_multi_val_free() on being dropped. (For internal use
    /// only.)
    pub fn do_not_free_on_drop(&mut self) {
        self.should_free_on_drop = false;
    }
}

impl Drop for MultiVal {
    fn drop(&mut self) {
        if self.should_free_on_drop {
            unsafe {
                isl_multi_val_free(self.ptr);
            }
        }
    }
}
